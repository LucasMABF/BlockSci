...
namespace blocksci {
  ...
  struct BLOCKSCI_EXPORT RawTransaction {
    ...
    RawTransaction(uint32_t realSize_, uint32_t baseSize_, uint32_t locktime_, uint16_t inputCount_,
                   uint16_t outputCount_)
        : realSize(realSize_), baseSize(baseSize_), locktime(locktime_), inputCount(inputCount_),
