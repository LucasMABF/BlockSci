...
namespace blocksci {
   ...
   *
   * Actual Input and Output objects can be constructed from Inout objects @see blocksci::Transaction::outputs() and
   * blocksci::Transaction::inputs()
   */
  struct BLOCKSCI_EXPORT Inout {
    Inout(uint32_t linkedTxNum_, uint32_t addressNum, AddressType::Enum type, int64_t value)
        : linkedTxNum(linkedTxNum_), toAddressNum(addressNum), other(0) {
      setValue(value);
      setType(type);
    }
    ...
    bool operator==(const Inout &otherInout) const {
      return linkedTxNum == otherInout.linkedTxNum && toAddressNum == otherInout.toAddressNum &&
             other == otherInout.other;
    }
