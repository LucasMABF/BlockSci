...
namespace blocksci {
  namespace internal {
    ...
    template <typename A, template <typename...> class O> struct to_variadic;
    ...
    template <typename EnumStruct, typename... Args> struct MakeDynamicTableImpl {
      template <template <typename EnumStruct::Enum, typename...> class Functor, std::size_t... Is>
      constexpr auto makeTable(std::index_sequence<Is...>)
          -> std::array<decltype(&Functor<EnumStruct::example, Args...>::f), sizeof...(Is)> {
        return {{Functor<std::tuple_element_t<Is, typename EnumStruct::all>::value, Args...>::f...}};
      }
    };

    template <typename EnumStruct, class... Args> struct MakeStaticTableImpl {
      template <template <typename EnumStruct::Enum> class Functor, std::size_t... Is>
      constexpr auto makeTable(std::index_sequence<Is...>, Args &&...args)
          -> std::array<decltype(Functor<EnumStruct::example>::f(std::forward<Args>(args)...)), sizeof...(Is)> {
        return {
            {Functor<std::tuple_element_t<Is, typename EnumStruct::all>::value>::f(std::forward<Args>(args)...)...}};
      }
    };

    template <typename Z, template <Z> class K, typename T> struct apply_template;

    template <typename Z, template <Z> class K, typename... Types> struct apply_template<Z, K, std::tuple<Types...>> {
      using type = std::tuple<K<Types::value>...>;
    };
  } // namespace internal

  template <class Tuple, class F> constexpr auto apply(Tuple &&t, F f) {
    return internal::index_apply<std::tuple_size<std::decay_t<Tuple>>{}>(
        [&](auto... Is) { return std::make_tuple(f(std::get<Is>(t))...); });
  }

  template <class Tuple, typename F> constexpr void for_each(Tuple &&t, F f) {
