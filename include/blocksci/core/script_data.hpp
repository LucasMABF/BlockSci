...
namespace blocksci {
  ...
  template <typename T, typename Index = uint32_t> class BLOCKSCI_EXPORT InPlaceArray {
    Index dataSize;
  ...
  struct BLOCKSCI_EXPORT ScriptDataBase {
    ...
    /** Transaction number where this script has been spent first; if not spent yet, defaults to
     * std::numeric_limits<uint32_t>::max() */
    uint32_t txFirstSpent;

    uint32_t typesSeen;

    explicit ScriptDataBase(uint32_t txNum)
        : txFirstSeen(txNum), txFirstSpent(std::numeric_limits<uint32_t>::max()), typesSeen(0) {
    }
  ...
  struct BLOCKSCI_EXPORT ScriptHashData : public ScriptDataBase {
    ...
    ScriptHashData(uint32_t txNum, uint160 hash160_, const RawAddress &wrappedAddress_)
        : ScriptDataBase(txNum), wrappedAddress(wrappedAddress_), isSegwit(false) {
      hash256.SetNull();
      hash160 = hash160_;
    }

    ScriptHashData(uint32_t txNum, uint256 hash256_, const RawAddress &wrappedAddress_)
        : ScriptDataBase(txNum), hash256(hash256_), wrappedAddress(wrappedAddress_), isSegwit(true) {
    }
  ...
  struct BLOCKSCI_EXPORT MultisigData : public ScriptDataBase {
    ...
    MultisigData(uint32_t txNum, uint8_t m_, uint8_t n_, uint16_t addressCount)
        : ScriptDataBase(txNum), m(m_), n(n_), addresses(addressCount) {
    }
  ...
  struct BLOCKSCI_EXPORT RawData : public ScriptDataBase {
    ...
    RawData(uint32_t txNum, const std::vector<unsigned char> &fullData)
        : ScriptDataBase(txNum), rawData(static_cast<uint32_t>(fullData.size())) {
    }
  };
  ...
  struct BLOCKSCI_EXPORT WitnessUnknownScriptData : public ScriptDataBase {
    ...
    WitnessUnknownScriptData(uint32_t txNum, uint8_t witnessVersion_, uint32_t scriptLength)
        : ScriptDataBase(txNum), witnessVersion(witnessVersion_), scriptData(scriptLength) {
    }
  };
