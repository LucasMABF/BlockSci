...
namespace blocksci {
  ...
  template <typename T> std::string BLOCKSCI_EXPORT HexStr(const T itbegin, const T itend, bool fSpaces = false) {
    std::string rv;
  ...
  template <typename T> inline std::string BLOCKSCI_EXPORT HexStr(const T &vch, bool fSpaces = false) {
  ...
  public:
    ...
    template <typename It> base_blob(It begin, It end) {
...
namespace std {
  template <> struct hash<blocksci::uint256> {
  public:
    size_t operator()(const blocksci::uint256 &b) const {
      return b.GetUint64(2);
    }
  };
  template <> struct hash<blocksci::uint160> {
  public:
