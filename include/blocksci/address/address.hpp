...
namespace blocksci {
  ...
  public:
    ...
    Address(uint32_t addressNum_, AddressType::Enum type_, DataAccess &access_)
        : access(&access_), scriptNum(addressNum_), type(type_) {
    }
...
namespace std {
  template <> struct hash<blocksci::Address> {
