...
namespace blocksci {
  ...
  struct BLOCKSCI_EXPORT OutputRange {
    ...
    struct iterator {
      ...
      iterator() = default;
      iterator(const Inout *inouts_, BlockHeight height_, uint32_t txIndex_, uint16_t outputNum_, uint32_t maxTxLoaded_,
               DataAccess *access_)
          : inouts(inouts_), height(height_), txIndex(txIndex_), outputNum(outputNum_), maxTxLoaded(maxTxLoaded_),
            access(access_) {
      }
      ...
      value_type operator[](difference_type i) const {
        return {{txIndex, static_cast<uint16_t>(static_cast<int>(outputNum) + i)},
                height,
                inouts[outputNum],
                maxTxLoaded,
                *access};
      ...
      bool operator==(const self_type &rhs) const {
        return outputNum == rhs.outputNum;
      }
      bool operator!=(const self_type &rhs) const {
        return outputNum != rhs.outputNum;
      }
      ...
      difference_type operator-(const self_type &it) const {
        return static_cast<int>(outputNum) - static_cast<int>(it.outputNum);
      }
    };
  ...
  inline OutputRange::iterator::self_type BLOCKSCI_EXPORT operator+(OutputRange::iterator::difference_type i,
                                                                    const OutputRange::iterator &it) {
    return it + i;
  }
...
namespace ranges {
  template <> inline constexpr bool enable_view<blocksci::OutputRange> = true;
} // namespace ranges
