...
namespace blocksci {
  ...
  struct BLOCKSCI_EXPORT InputRange {
    /** Pointer to the Inout of the first input in the range, more <Inout>s follow and can be accessed like
     * inouts[number] */
    const Inout *inouts;
    ...
    struct iterator {
      ...
      /** Parameters of Input constructor: InputPointer &pointer_, BlockHeight blockHeight_, Inout &inout_, uint16_t
       * *spentOutputNum_, uint32_t *sequenceNum_, uint32_t maxTxCount_, DataAccess &access_ */
      value_type operator*() const {
        return {{txIndex, inputNum},        height,     inouts[inputNum], &spentOutputNums[inputNum],
                &sequenceNumbers[inputNum], maxTxCount, *access};
      }
      value_type operator[](difference_type i) const {
        auto index = static_cast<uint16_t>(static_cast<int>(inputNum) + i);
        return {{txIndex, index},        height,     inouts[index], &spentOutputNums[inputNum],
                &sequenceNumbers[index], maxTxCount, *access};
      }

      bool operator==(const self_type &rhs) const {
        return inputNum == rhs.inputNum;
      }
      bool operator!=(const self_type &rhs) const {
        return inputNum != rhs.inputNum;
      }
      ...
      difference_type operator-(const self_type &it) const {
        return static_cast<int>(inputNum - it.inputNum);
      }
    };
  ...
  inline InputRange::iterator::self_type BLOCKSCI_EXPORT operator+(InputRange::iterator::difference_type i,
                                                                   const InputRange::iterator &it) {
    return it + i;
  }
