...
namespace blocksci {
  ...
  public:
    Block() = default;
    Block(const RawBlock *rawBlock_, const Transaction &firstTx)
        : TransactionRange({rawBlock_->firstTxIndex, rawBlock_->firstTxIndex + rawBlock_->txCount}, firstTx),
          rawBlock(rawBlock_) {
    }
...
namespace std {
  template <> struct hash<blocksci::Block> {
