...
namespace blocksci {
  ...
  template <typename T> struct fail_helper : std::false_type {};
  ...
  inline decltype(auto) BLOCKSCI_EXPORT inputs(B &&b) {
    if constexpr (isInputRange<B>) {
      ...
      return txes(std::forward<B>(b)) | ranges::views::transform([](const Transaction &tx) { return tx.inputs(); }) |
             ranges::views::join;
    }
  }
  ...
  inline decltype(auto) BLOCKSCI_EXPORT outputs(B &&b) {
    if constexpr (isOutputRange<B>) {
      ...
      return txes(std::forward<B>(b)) | ranges::views::transform([](const Transaction &tx) { return tx.outputs(); }) |
             ranges::views::join;
    }
  }
  ...
  template <typename T> inline auto BLOCKSCI_EXPORT outputsUnspent(T &&t) {
    return outputs(std::forward<T>(t)) |
           ranges::views::remove_if([](const Output &output) { return output.isSpent(); });
  }

  template <typename T> inline auto BLOCKSCI_EXPORT outputsSpentBeforeHeight(T &&t, blocksci::BlockHeight blockHeight) {
    return outputs(std::forward<T>(t)) | ranges::views::filter([=](const Output &output) {
             return output.isSpent() && output.getSpendingTx()->getBlockHeight() < blockHeight;
           });
  }

  template <typename T> inline auto BLOCKSCI_EXPORT outputsSpentAfterHeight(T &&t, blocksci::BlockHeight blockHeight) {
    return outputs(std::forward<T>(t)) | ranges::views::filter([=](const Output &output) {
             return output.isSpent() && output.getSpendingTx()->getBlockHeight() >= blockHeight;
           });
  }

  template <typename T> inline auto BLOCKSCI_EXPORT inputsCreatedAfterHeight(T &&t, blocksci::BlockHeight blockHeight) {
    return inputs(std::forward<T>(t)) | ranges::views::filter([=](const Input &input) {
             return input.getSpentTx().getBlockHeight() >= blockHeight;
           });
  }

  template <typename T>
  inline auto BLOCKSCI_EXPORT inputsCreatedBeforeHeight(T &&t, blocksci::BlockHeight blockHeight) {
    return inputs(std::forward<T>(t)) |
           ranges::views::filter([=](const Input &input) { return input.getSpentTx().getBlockHeight() < blockHeight; });
  }
  ...
  inline auto BLOCKSCI_EXPORT outputsSpentOutsideRelativeHeight(T &&t, blocksci::BlockHeight difference) {
    return outputs(std::forward<T>(t)) | ranges::views::filter([=](const Output &output) {
             return output.isSpent() &&
                    output.getSpendingTx()->getBlockHeight() - output.getBlockHeight() >= difference;
           });
  }
  ...
  template <typename T> inline auto BLOCKSCI_EXPORT outputsOfType(T &&t, AddressType::Enum type) {
    return outputs(std::forward<T>(t)) |
           ranges::views::filter([=](const Output &output) { return output.getType() == type; });
  }

  template <typename T> inline auto BLOCKSCI_EXPORT inputsOfType(T &&t, AddressType::Enum type) {
    return inputs(std::forward<T>(t)) |
           ranges::views::filter([=](const Input &input) { return input.getType() == type; });
  }

  template <typename T> inline uint64_t BLOCKSCI_EXPORT inputCount(T &&t) {
    auto values =
        txes(std::forward<T>(t)) | ranges::views::transform([](const Transaction &tx) { return tx.inputCount(); });
    return ranges::accumulate(values, uint64_t{0});
  }

  template <typename T> inline uint64_t BLOCKSCI_EXPORT outputCount(T &&t) {
    auto values =
        txes(std::forward<T>(t)) | ranges::views::transform([](const Transaction &tx) { return tx.outputCount(); });
    return ranges::accumulate(values, uint64_t{0});
  }

  template <typename T> inline int64_t BLOCKSCI_EXPORT totalInputValue(T &&t) {
  ...
  template <typename T> inline int64_t BLOCKSCI_EXPORT totalOutputValue(T &&t) {
  ...
  template <typename T> inline int64_t BLOCKSCI_EXPORT balance(BlockHeight height, T &&t) {
    int64_t value = 0;
    if (height == -1) {
      ...
      RANGES_FOR(auto output, t) {
        if (output.getBlockHeight() <= height &&
            (!output.isSpent() || output.getSpendingTx()->getBlockHeight() > height)) {
          value += output.getValue();
        }
      }
    }
  ...
  template <typename T> inline auto BLOCKSCI_EXPORT feeLessThan(T &t, int64_t value) {
    return txes(t) | ranges::views::filter([=](const Transaction &tx) { return fee(tx) < value; });
  }

  template <typename T> inline auto BLOCKSCI_EXPORT feeGreaterThan(T &t, int64_t value) {
    return txes(t) | ranges::views::filter([=](const Transaction &tx) { return fee(tx) > value; });
  }

  template <typename T> inline auto BLOCKSCI_EXPORT fees(T &t) {
    return ranges::views::transform(txes(t), [](Transaction &&tx) { return fee(tx); });
  }
  ...
  template <typename T> inline auto BLOCKSCI_EXPORT feesPerByte(T &t) {
    return ranges::views::transform(txes(t), feePerByte);
  }

  template <typename T> inline int64_t BLOCKSCI_EXPORT totalFee(T &t) {
