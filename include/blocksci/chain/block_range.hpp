...
namespace blocksci {
  ...
  namespace internal {
    template <typename F, typename... Args> struct BLOCKSCI_EXPORT is_callable {
    ...
    ResultType BLOCKSCI_EXPORT mapReduceBlocksImp(It begin, It end, MapFunc mapFunc, ReduceFunc reduceFunc,
                                                  int segmentBeginNum) {
      ...
      if (segmentCount == 1) {
        ...
        auto handle = std::async(std::launch::async, mapReduceBlocksImp<ResultType, It, MapFunc, ReduceFunc>, begin,
                                 mid, mapFunc, reduceFunc, segmentBeginNum);
        ResultType res{};
  ...
  public:
    ...
    public:
      ...
      self_type operator+(difference_type i) const {
        self_type tmp = *this;
        tmp.height += i;
        return tmp;
      }
      ...
      value_type operator*() const {
        return Block(height, *access);
      }
      value_type operator[](difference_type i) const {
        return Block(height + i, *access);
      }
      ...
      BlockHeight operator-(const self_type &it) const {
        return height - it.height;
      }
    ...
    template <typename ResultType, typename MapFunc, typename ReduceFunc>
    std::enable_if_t<internal::is_callable<MapFunc, BlockRange>::value, ResultType> mapReduce(MapFunc mapFunc,
                                                                                              ReduceFunc reduceFunc) {
      auto segments = segment(std::thread::hardware_concurrency());
      return internal::mapReduceBlocksImp<ResultType>(
          segments.begin(), segments.end(), [&](const BlockRange &blocks, int) { return mapFunc(blocks); }, reduceFunc,
          0);
    }

    template <typename ResultType, typename MapFunc, typename ReduceFunc>
    std::enable_if_t<internal::is_callable<MapFunc, Block>::value, ResultType> mapReduce(MapFunc mapFunc,
                                                                                         ReduceFunc reduceFunc) {
    ...
    template <typename ResultType, typename MapFunc, typename ReduceFunc>
    std::enable_if_t<internal::is_callable<MapFunc, Transaction>::value, ResultType> mapReduce(MapFunc mapFunc,
                                                                                               ReduceFunc reduceFunc) {
    ...
    template <typename MapType> std::vector<MapType> map(const std::function<MapType(const Block &)> &mapFunc) {
      auto mapF = [&](const BlockRange &segment) {
    ...
    // Returns a vector of [start, stop) intervals splitting the chain into segments with approximately the same number
    // of segments
  ...
  inline std::vector<Transaction> BLOCKSCI_EXPORT getTransactionsIncludingOutput(BlockRange &chain,
                                                                                 AddressType::Enum type) {
    return chain.filter([type](const Transaction &tx) { return includesOutputOfType(tx, type); });
  }

  inline BlockRange::iterator::self_type BLOCKSCI_EXPORT operator+(BlockRange::iterator::difference_type i,
                                                                   const BlockRange::iterator &it) {
    return it + i;
  }
