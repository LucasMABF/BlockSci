...
namespace blocksci {
  ...
  public:
    /** Blockchain-wide transaction number in the same order they appear in the blockchain, also called transaction
     * index */
    uint32_t txNum;

    Transaction() = default;

    Transaction(const TxData &data_, uint32_t txNum_, BlockHeight blockHeight_, uint32_t maxTxCount_,
                DataAccess &access_)
        : access(&access_), data(data_), maxTxCount(maxTxCount_), blockHeight(blockHeight_), txNum(txNum_) {
    }
    ...
    InputRange inputs() const {
      return {data.rawTx->beginInputs(),
              data.spentOutputNums,
...
namespace std {
  template <> struct BLOCKSCI_EXPORT hash<blocksci::Transaction> {
