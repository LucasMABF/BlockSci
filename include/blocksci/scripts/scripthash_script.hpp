...
namespace blocksci {
  ...
  protected:
    ScriptHashBase(uint32_t scriptNum_, AddressType::Enum type_, const ScriptHashData *rawData_, DataAccess &access_)
        : ScriptBase(scriptNum_, type_, access_, rawData_) {
    }
  ...
  template <> class BLOCKSCI_EXPORT ScriptAddress<AddressType::SCRIPTHASH> : public ScriptHashBase {
  public:
    constexpr static AddressType::Enum addressType = AddressType::SCRIPTHASH;

    ScriptAddress(uint32_t addressNum_, const ScriptHashData *data_, DataAccess &access_)
        : ScriptHashBase(addressNum_, addressType, data_, access_) {
    }
  ...
  template <> class BLOCKSCI_EXPORT ScriptAddress<AddressType::WITNESS_SCRIPTHASH> : public ScriptHashBase {
  public:
