...
namespace blocksci {
  ...
  public:
    ...
    static ClusterManager createClustering(BlockRange &chain, const heuristics::ChangeHeuristic &heuristic,
                                           const std::string &outputPath, bool overwrite = false,
                                           bool ignoreCoinJoin = true);
    static ClusterManager
    createClustering(BlockRange &chain,
                     const std::function<ranges::any_view<Output>(const Transaction &tx)> &changeHeuristic,
