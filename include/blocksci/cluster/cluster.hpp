...
namespace blocksci {
  ...
  struct AddressRangeTagChecker {
    ...
    auto
    operator()(ranges::transform_view<ranges::subrange<const AddressType::Enum *>, ClusterAddressCreator> &&rng) const {
  ...
  class BLOCKSCI_EXPORT Cluster {
    ...
    taggedAddressesUnsafe(const std::unordered_map<blocksci::Address, std::string> &tags) const;
  ...
  public:
    ...
    ranges::any_view<TaggedAddress>
    taggedAddresses(const std::unordered_map<blocksci::Address, std::string> &tags) const;
    ...
    getTaggedUnsafe(const std::unordered_map<blocksci::Address, std::string> &tags) const;
  ...
  struct BLOCKSCI_EXPORT TaggedCluster {
    Cluster cluster;
    using TaggedRange =
        decltype(std::declval<Cluster>().taggedAddressesNested(std::unordered_map<blocksci::Address, std::string>{}));
  ...
  private:
    ...
    TaggedCluster(const Cluster &cluster_, TaggedRange &&taggedAddresses_)
        : cluster(cluster_), taggedAddresses(std::move(taggedAddresses_)) {
    }
  };
