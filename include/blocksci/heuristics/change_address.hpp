...
#define CHANGE_ADDRESS_TYPE_LIST                                                                                       \
  VAL(PeelingChain), VAL(PowerOfTen), VAL(OptimalChange), VAL(AddressType), VAL(Locktime), VAL(AddressReuse),          \
      VAL(ClientChangeAddressBehavior), VAL(Legacy), VAL(FixedFee), VAL(None), VAL(Spent)
#define CHANGE_ADDRESS_TYPE_SET                                                                                        \
  VAL(PeelingChain), VAL(PowerOfTen), VAL(OptimalChange), VAL(AddressType) VAL(Locktime), VAL(AddressReuse),           \
      VAL(ClientChangeAddressBehavior), VAL(Legacy), VAL(FixedFee), VAL(None), VAL(Spent)
namespace blocksci {
...
#undef VAL
    ...
    template <ChangeType::Enum heuristic> struct BLOCKSCI_EXPORT ChangeHeuristicImpl {
      ranges::any_view<Output> operator()(const Transaction &tx) const;
    };

    template <> struct BLOCKSCI_EXPORT ChangeHeuristicImpl<ChangeType::PowerOfTen> {
      int digits;
    ...
    struct BLOCKSCI_EXPORT ChangeHeuristic {
      ...
      template <typename T> ChangeHeuristic(T func) : impl(std::move(func)) {
      }
