...
enum class ProxyType { Simple, Optional, Iterator, Range };
...
template <typename T> using RawIterator = ranges::any_view<T>;

template <typename T> using RawRange = ranges::any_view<T, random_access_sized>;
...
template <typename T, typename SimpleBase = SimpleProxy> struct AllProxyClasses;
