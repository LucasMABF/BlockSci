...
struct AddEquivAddressMethods {
  template <typename FuncApplication> void operator()(FuncApplication func) {
    ...
    func(
        property_tag, "outputs",
        +[](const EquivAddress &address) -> RawIterator<Output> {
          return ranges::views::ints(0, 1) | ranges::views::transform([address](int) { return address.getOutputs(); }) |
                 ranges::views::join;
        },
    ...
    func(
        property_tag, "inputs",
        +[](EquivAddress &address) -> RawIterator<Input> {
          return ranges::views::ints(0, 1) | ranges::views::transform([address](int) { return address.getInputs(); }) |
                 ranges::views::join;
        },
    ...
    func(
        ...
        +[](EquivAddress &address) -> int64_t { return address.getInputTransactions().size(); },
