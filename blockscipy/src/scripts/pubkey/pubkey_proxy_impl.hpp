...
template <typename T> struct AddPubkeyBaseMethods {
  template <typename FuncApplication> void operator()(FuncApplication func) {
    using namespace blocksci;
    func(
        method_tag, "find_multisigs",
        +[](const T &script) -> RawIterator<script::Multisig> {
    ...
    func(
        property_tag, "pubkey",
        +[](const T &script) -> ranges::optional<pybind11::bytes> {
