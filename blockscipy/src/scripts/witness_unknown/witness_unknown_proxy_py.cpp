...
struct AddWitnessUnknownMethods {
  template <typename FuncApplication> void operator()(FuncApplication func) {
    func(
        property_tag, "witness_version",
        +[](const script::WitnessUnknown &script) -> int64_t { return script.witnessVersion(); },
        "Witness version of the unknown script");
    ...
    func(
        ...
        +[](const script::WitnessUnknown &script) -> ranges::optional<py::list> {
