...
namespace pybind11 {
  namespace detail {
    template <> struct npy_format_descriptor<NumpyDatetime> {
      static constexpr auto name = _("datetime64[ns]");
      static pybind11::dtype dtype() {
        return pybind11::dtype(std::string("datetime64[ns]"));
      }
    };

    template <> struct npy_format_descriptor<NumpyBool> {
      static constexpr auto name = _("bool");
      static pybind11::dtype dtype() {
        return pybind11::dtype(std::string("bool"));
      }
    };
  } // namespace detail
} // namespace pybind11
...
struct PythonConversionTypeConverter {
  ...
  template <typename T> Iterator<T> operator()(RawIterator<T> &&t) {
    return t;
  }

  template <typename T> Range<T> operator()(RawRange<T> &&t) {
    return t;
  }
};

template <typename T> auto convertPythonRange(T &&t) {
