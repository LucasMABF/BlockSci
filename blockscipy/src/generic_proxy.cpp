...
std::function<std::any(std::any &)> IteratorProxy::getGenericAny() const {
  ...
  return [generic](std::any &val) -> std::any { return generic(val).toAny(); };
}

std::function<BlocksciIteratorType(std::any &)> RangeProxy::getGenericIterator() const {
  return [generic = getGenericRange()](std::any &val) -> BlocksciIteratorType { return generic(val).toIterator(); };
}
...
std::function<ranges::optional<BlocksciType>(std::any &)> SimpleProxy::getGenericOptional() const {
  return [generic = getGenericSimple()](std::any &val) -> ranges::optional<BlocksciType> { return generic(val); };
}
...
std::function<BlocksciRangeType(std::any &)> SimpleProxy::getGenericRange() const {
  return [generic = getGenericSimple()](std::any &v) -> BlocksciRangeType {
    return mpark::visit(
        [](auto &&val) -> BlocksciRangeType {
          return BlocksciRangeType{
              RawRange<std::decay_t<decltype(val)>>{ranges::views::single(std::forward<decltype(val)>(val))}};
        },
        generic(v).var);
  };
}
...
std::function<BlocksciType(std::any &)> ProxyAddress::getGenericSimple() const {
  return [generic = getGenericScript()](std::any &val) -> BlocksciType { return BlocksciType{generic(val)}; };
}
