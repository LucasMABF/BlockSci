...
OtherProxies::OtherProxies(py::module &m)
    : uint256(createProxyClasses<blocksci::uint256>(m)), uint160(createProxyClasses<blocksci::uint160>(m)),
      addressType(createProxyClasses<AddressType::Enum>(m)), integer(createProxyClasses<int64_t>(m)),
      boolean(createProxyClasses<bool>(m)), time(createProxyClasses<std::chrono::system_clock::time_point>(m)),
      string(createProxyClasses<std::string>(m)), bytes(createProxyClasses<py::bytes>(m)),
