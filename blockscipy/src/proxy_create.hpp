...
template <typename T> struct SimpleProxyCreator {
  Proxy<T> operator()() const {
    return {std::function<T(std::any &)>{[](std::any &t) -> T { return std::any_cast<T>(t); }},
            createProxyTypeInfo<T>()};
  }
};

template <> struct SimpleProxyCreator<blocksci::AnyScript> {
  Proxy<blocksci::AnyScript> operator()() const;
};

template <typename T> Proxy<T> makeSimpleProxy() {
  return SimpleProxyCreator<T>{}();
}

template <typename T> Proxy<ranges::optional<T>> makeOptionalProxy() {
...
template <typename T> Proxy<RawIterator<T>> makeIteratorProxy() {
...
template <typename T> Proxy<RawRange<T>> makeRangeProxy() {
