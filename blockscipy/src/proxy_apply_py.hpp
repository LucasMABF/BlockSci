...
template <typename P, typename Out, typename R> struct ApplyMethodsToProxyFuncBinder {
...
template <typename P, typename Out, typename R, typename... Args> struct ApplyMethodsToProxyFuncConverter {
  ...
  auto operator()(P &p, const Args &...args) const
      -> decltype(lift(p, proxy_apply_converter_t<P, Out, R>{std::bind(func, std::placeholders::_1, args...)})) {
    return lift(p, proxy_apply_converter_t<P, Out, R>{std::bind(func, std::placeholders::_1, args...)});
  }
};

template <typename P, typename Out, typename R, typename... Args> struct ApplyGenericMethodsToProxyFuncConverter {
  using Func = std::function<R(Out &, Args...)>;
  Func func;
  ...
  auto operator()(P &p, const Args &...args) const
      -> decltype(liftGeneric(p, proxy_apply_converter_t<P, Out, R>{std::bind(func, std::placeholders::_1, args...)})) {
...
struct ApplyMethodsToProxyImpl {
  ...
  void applyPropertyImpl(const std::string &propertyName, const pybind11::cpp_function &func,
  ...
  void applyProperty(const std::string &propertyName, std::function<result_type(Out &)> func,
                     const std::string &description) {
    using converted_t = Converter<P, Out, result_type>;
    converted_t convertedFunc{func};
    applyPropertyImpl(
        propertyName,
        pybind11::cpp_function(std::move(convertedFunc), pybind11::return_value_policy::reference_internal),
        strdup(description.c_str()));
  }

  template <typename result_type, typename... Args, typename... Extra>
  void applyMethod(const std::string &propertyName, std::function<result_type(Out &, Args...)> func,
                   const std::string &description, Extra &&...extra) {
  ...
  void operator()(method_tag_type, const std::string &propertyName, F func, const std::string &description,
                  Extra &&...extra) {
...
template <typename Class, typename Applier> void applyMethodsToProxy(Class &cl, Applier applier) {
  using P = typename Class::type;
  using Out = typename P::output_t;
  applier(ApplyMethodsToProxyImpl<ApplyMethodsToProxyFuncConverter, Out, Class>{cl});
}

template <typename Class, typename Applier> void applyMethodsToProxyGeneric(Class &cl, Applier applier) {
  using P = typename Class::type;
