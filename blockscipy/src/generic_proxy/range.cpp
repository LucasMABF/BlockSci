...
void applyProxyIteratorFuncs(pybind11::class_<IteratorProxy, GenericProxy> &cl, pybind11::module &m) {
  ...
  cl.def_property_readonly("size",
                           [](IteratorProxy &p) -> Proxy<int64_t> {
                             return liftGeneric(p, [](auto &&seq) -> int64_t {
                               return mpark::visit(ranges::distance, std::forward<decltype(seq)>(seq).var);
                             });
                           })
      .def("_any",
           [](IteratorProxy &p, Proxy<bool> &p2) -> Proxy<bool> {
             return liftGeneric(p, [p2](auto &&seq) -> bool {
               return mpark::visit(
                   [p2](auto &&r) -> bool {
                     return ranges::any_of(std::forward<decltype(r)>(r),
                                           [p2](auto &&item) { return p2(std::forward<decltype(item)>(item)); });
                   },
      ...
      .def("_all", [](IteratorProxy &p, Proxy<bool> &p2) -> Proxy<bool> {
        return liftGeneric(p, [p2](auto &&seq) -> bool {
          return mpark::visit(
              [p2](auto &&r) -> bool {
                return ranges::all_of(std::forward<decltype(r)>(r),
                                      [p2](auto &&item) { return p2(std::forward<decltype(item)>(item)); });
              },
              std::forward<decltype(seq)>(seq).var);
        });
...
void applyProxyRangeFuncs(pybind11::class_<RangeProxy, IteratorProxy> &cl) {
  ...
  cl.def_property_readonly("size", [](RangeProxy &p) -> Proxy<int64_t> {
    return liftGeneric(
        p, [](auto &&seq) -> int64_t { return mpark::visit(ranges::distance, std::forward<decltype(seq)>(seq).var); });
  });
}
