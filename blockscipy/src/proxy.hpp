...
template <typename T> struct SequenceProxy {
...
template <ranges::category range_cat> struct SequenceProxyType;

template <> struct SequenceProxyType<ranges::category::input> {
  using type = IteratorProxy;
};

template <> struct SequenceProxyType<random_access_sized> {
  using type = RangeProxy;
};

template <ranges::category range_cat> using proxy_sequence = typename SequenceProxyType<range_cat>::type;

template <typename T> struct Proxy : public SimpleProxy {
  using output_t = T;

  std::function<output_t(std::any &)> func;
  ProxyTypeInfo sourceType;

  Proxy(std::function<output_t(std::any &)> &&func_, const ProxyTypeInfo &sourceType_)
      : func(std::move(func_)), sourceType(sourceType_) {
  }
  ...
  std::function<BlocksciType(std::any &)> getGenericSimple() const override {
    return [f = this->func](std::any &val) -> BlocksciType { return BlocksciType{f(val)}; };
  }
...
template <typename T> struct Proxy<ranges::optional<T>> : public OptionalProxy {
  ...
  std::function<std::any(std::any &)> getGenericAny() const override {
    return [f = this->func](std::any &val) -> std::any { return f(val); };
  }
...
template <typename T> struct Proxy<RawRange<T>> : public SequenceProxy<T>, public RangeProxy {
  ...
  std::function<BlocksciRangeType(std::any &)> getGenericRange() const override {
    return [f = this->func](std::any &val) -> BlocksciRangeType { return BlocksciRangeType{f(val)}; };
  }
...
template <typename T> struct Proxy<RawIterator<T>> : public SequenceProxy<T>, public IteratorProxy {
  ...
  std::function<BlocksciIteratorType(std::any &)> getGenericIterator() const override {
    return [f = this->func](std::any &val) -> BlocksciIteratorType { return BlocksciIteratorType{f(val)}; };
  }
...
template <blocksci::AddressType::Enum type> struct Proxy<blocksci::ScriptAddress<type>> : public ProxyAddress {
  ...
  std::function<blocksci::AnyScript(std::any &)> getGenericScript() const override {
    return [f = this->func](std::any &val) -> blocksci::AnyScript { return blocksci::ScriptVariant{f(val)}; };
  }
...
template <> struct Proxy<blocksci::AnyScript> : public ProxyAddress {
