...
template <AddressType::Enum type> using PythonScriptRange = Range<ScriptAddress<type>>;
using PythonScriptRangeVariant = to_variadic_t<to_address_tuple_t<PythonScriptRange>, mpark::variant>;

namespace {
  template <blocksci::AddressType::Enum type> struct PythonScriptRangeFunctor {
...
void init_blockchain(py::class_<Blockchain> &cl) {
  ...
  cl.def(py::init<std::string>())
      ...
      .def(
          "tx_with_index", [](Blockchain &chain, uint32_t index) { return Transaction{index, chain.getAccess()}; },
          "This functions gets the transaction with given index.", pybind11::arg("index"))
      .def(
          "tx_with_hash",
          [](Blockchain &chain, const std::string &hash) { return Transaction{hash, chain.getAccess()}; },
