...
void init_uint256(py::class_<uint256> &cl) {
  cl.def("__repr__", &uint256::GetHex)
      .def(py::pickle(
          [](const uint256 &hash) {
...
void init_uint160(py::class_<uint160> &cl) {
  cl.def("__repr__", &uint160::GetHex)
      .def(py::pickle(
          [](const uint160 &hash) {
            std::vector<char> data;
            for (auto c : hash) {
...
void init_block(py::class_<Block> &cl) {
  ...
  cl.def("__repr__", &Block::toString)
      .def(py::self == py::self)
      .def(hash(py::self))
      .def_property_readonly("_access", [](const Block &block) { return Access{&block.getAccess()}; })
      .def("net_address_type_value", py::overload_cast<const Block &>(netAddressTypeValue),
           "Returns a set of the net change in the utxo pool after this block split up by address type")
      .def("net_full_type_value", py::overload_cast<const Block &>(netFullTypeValue),
           "Returns a set of the net change in the utxo pool after this block split up by full type");

  cl.def(
        "__iter__", [](Block &range) { return pybind11::make_iterator(range.begin(), range.end()); },
        pybind11::keep_alive<0, 1>())
      .def("__bool__", [](Block &range) { return !ranges::empty(range); })
      .def("__len__", [](Block &range) { return range.size(); })
      .def(
          "to_list",
          [](Block &range) {
            py::list list;
