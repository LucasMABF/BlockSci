...
struct AddBlockProxyMethods {
  template <typename FuncApplication> void operator()(FuncApplication func) {
    ...
    func(
        property_tag, "txes",
        +[](const Block &block) -> RawRange<Transaction> {
          return ranges::any_view<blocksci::Transaction, random_access_sized>{block};
        },
        "A range of all of the txes in the block");
    func(
        property_tag, "inputs",
        +[](const Block &block) -> RawIterator<Input> { return ranges::any_view<blocksci::Input>{inputs(block)}; },
        "A range of all of the inputs in the block");
    func(
        property_tag, "outputs",
        +[](const Block &block) -> RawIterator<Output> { return ranges::any_view<blocksci::Output>{outputs(block)}; },
