...
void init_tx(py::class_<Transaction> &cl) {
  ...
  cl.def("__str__", &Transaction::toString)
      .def("__repr__", &Transaction::toString)
      .def(py::self == py::self)
      .def(hash(py::self))
      .def_property_readonly("_access", [](const Transaction &tx) { return Access{&tx.getAccess()}; })
      .def(py::init([](uint32_t index, blocksci::Blockchain &chain) { return Transaction{index, chain.getAccess()}; }),
