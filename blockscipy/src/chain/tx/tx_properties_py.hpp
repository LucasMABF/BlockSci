...
struct AddTransactionMethods {
  template <typename FuncApplication> void operator()(FuncApplication func) {
    ...
    func(property_tag, "block_height", &Transaction::getBlockHeight,
         "The height of the block that this transaction was in");
    func(
        property_tag, "block_time",
        +[](const Transaction &tx) -> std::chrono::system_clock::time_point { return tx.block().getTime(); },
        "The time that the block containing this transaction arrived");
    func(property_tag, "observed_in_mempool", &Transaction::observedInMempool,
         "Returns whether this transaction was seen in the mempool by the recorder");
    func(property_tag, "time_seen", &Transaction::getTimeSeen,
         "If recorded by the mempool recorder, the time that this transaction was first seen by your node");
    func(property_tag, "timestamp_seen", &Transaction::getTimestampSeen,
         "If recorded by the mempool recorder, the time that this transaction was first seen by your node");
    func(property_tag, "block", &Transaction::block, "The block that this transaction was in");
    func(
        property_tag, "index", +[](const Transaction &tx) { return tx.txNum; },
    ...
    func(
        property_tag, "fee", +[](const Transaction &tx) -> int64_t { return fee(tx); },
        "The fee paid by this transaction");
    func(
        method_tag, "fee_per_byte",
        +[](const Transaction &tx, const std::string &sizeMeasure) -> int64_t {
          auto txFee = fee(tx);
        ...
        "The (rounded) ratio of fee paid to size of this transaction (in byte). By default this uses virtual size, but "
        "passing total, base, weight, or virtual let's you choose a different size measure",
        pybind11::arg("size_measure") = "virtual");
    func(
        method_tag, "fee_per_kbyte",
        +[](const Transaction &tx, const std::string &sizeMeasure) -> int64_t {
          auto txFee = fee(tx);
          if (sizeMeasure == "total") {
        ...
        "The (rounded) ratio of fee paid to size of this transaction (in kbyte). By default this uses virtual size, "
        "but passing total, base, weight, or virtual let's you choose a different size measure",
        pybind11::arg("size_measure") = "virtual");
    func(
        property_tag, "op_return", +[](const Transaction &tx) -> ranges::optional<Output> { return getOpReturn(tx); },
        "If this transaction included a null data address, return its output. Otherwise return None");
    func(method_tag, "includes_output_of_type", includesOutputOfType,
         "Check whether the given transaction includes an output of the given address type",
         pybind11::arg("address_type"));
    func(property_tag, "is_coinbase", &Transaction::isCoinbase,
