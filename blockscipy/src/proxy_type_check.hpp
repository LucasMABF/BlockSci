...
template <typename T> struct ProxyTypeInfoCreator {
  ProxyTypeInfo operator()() const {
    return {&typeid(T), &typeid(T), ProxyType::Simple};
  }
};

template <typename T> struct ProxyTypeInfoCreator<ranges::optional<T>> {
  ProxyTypeInfo operator()() const {
    return {&typeid(ranges::optional<T>), &typeid(T), ProxyType::Optional};
  }
};

template <typename T> struct ProxyTypeInfoCreator<RawIterator<T>> {
  ProxyTypeInfo operator()() const {
    return {&typeid(RawIterator<T>), &typeid(T), ProxyType::Iterator};
  }
};

template <typename T> struct ProxyTypeInfoCreator<RawRange<T>> {
  ProxyTypeInfo operator()() const {
    return {&typeid(RawRange<T>), &typeid(T), ProxyType::Range};
  }
};

template <typename T> inline ProxyTypeInfo createProxyTypeInfo() {
