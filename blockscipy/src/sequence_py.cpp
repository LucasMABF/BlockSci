...
void addCommonRangeMethods(pybind11::class_<GenericRange, GenericIterator> &cl) {
  cl.def("__bool__", [](GenericRange &range) {
      return !range.empty();
    }).def("__len__", [](GenericRange &range) { return range.size(); });
}
...
struct GroupByVisitor {
  ...
  template <typename T> pybind11::dict operator()(RawIterator<T> &&rng) const {
...
struct ToListVisitor {
  template <typename T> pybind11::list operator()(RawIterator<T> &&rng) const {
    pybind11::list list;
    RANGES_FOR(auto &&item, rng) {
...
void addCommonIteratorMethods(pybind11::class_<GenericIterator> &cl) {
  cl.def("_group_by",
         [](GenericIterator &range, SimpleProxy &grouper, SimpleProxy &eval) -> pybind11::dict {
           return mpark::visit(GroupByVisitor{grouper, eval}, range.getGenericIterator().var);
         })
      .def(
          "to_list",
          [](GenericIterator &range) { return mpark::visit(ToListVisitor{}, range.getGenericIterator().var); },
