...
template <typename T> void addProxyFunctionsT(pybind11::module &m) {
  m.def("conditional", [](const Proxy<bool> &cond, const Proxy<T> &p1, const Proxy<T> &p2) -> Proxy<T> {
...
template <typename T> void addProxyFunctions(pybind11::module &, pybind11::module &pm) {
  ...
  pm.def("conditional", [](const Proxy<bool> &cond, const Proxy<ranges::optional<T>> &p) -> Proxy<ranges::optional<T>> {
