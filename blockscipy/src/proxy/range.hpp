...
template <typename T, typename BaseSimple> void setupRangesProxy(AllProxyClasses<T, BaseSimple> &cls) {
  cls.sequence
      .def("_where",
           [](SequenceProxy<T> &p, Proxy<bool> &p2) -> Proxy<RawIterator<T>> {
             return liftSequence(p, [p2](auto &&seq) -> RawIterator<T> {
               return ranges::views::filter(std::forward<decltype(seq)>(seq),
                                            [p2](T item) { return p2(std::move(item)); });
             });
           })
      .def("_max",
           [](SequenceProxy<T> &p, Proxy<int64_t> &p2) -> Proxy<ranges::optional<T>> {
  ...
  cls.range
      .def(
          "__getitem__",
          [](Proxy<RawRange<T>> &p, int64_t posIndex) -> Proxy<T> {
      ...
      .def(
          "__getitem__",
          [](Proxy<RawRange<T>> &p, pybind11::slice slice) -> Proxy<RawRange<T>> {
