...
template <typename Class> void addProxyBooleanMethods(Class &cl) {
  ...
  cl.def("__and__",
         [](P &p1, P &p2) -> P {
           ...
           return {std::function<bool(std::any &)>{[p1, p2](std::any &v) -> bool { return p1(v) && p2(v); }},
                   p1.getSourceType()};
         })
      .def("__or__",
           [](P &p1, P &p2) -> P {
      ...
      .def("__invert__",
           [](P &p) -> P { return lift(p, [](auto &&v) -> T { return !std::forward<decltype(v)>(v); }); });
}
