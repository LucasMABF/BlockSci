...
RawIterator<std::any> GenericIterator::getAllIterator() {
  return mpark::visit(
      [](auto &&seq) -> RawIterator<std::any> {
        return ranges::views::transform(std::forward<decltype(seq)>(seq),
                                        [](auto &&item) -> std::any { return std::forward<decltype(item)>(item); });
      },
