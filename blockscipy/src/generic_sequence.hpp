...
template <typename T, ranges::category range_cat> struct AnyViewType;

struct BlocksciIteratorType;
struct BlocksciRangeType;

template <typename T> struct AnyViewType<T, ranges::category::input> {
  using type = Iterator<T>;
};

template <typename T> struct AnyViewType<T, random_access_sized> {
  using type = Range<T>;
};

template <typename T, ranges::category range_cat> using any_view = typename AnyViewType<T, range_cat>::type;
