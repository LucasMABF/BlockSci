...
template <typename Class> struct ApplyMethodsToSelfImpl {
  ...
  void operator()(property_tag_type, const std::string &propertyName, F func, const std::string &description,
                  Extra &&...extra) {
    cl.def_property_readonly(strdup(propertyName.c_str()),
                             pybind11::cpp_function(std::move(func), pybind11::return_value_policy::reference_internal),
                             strdup(description.c_str()));
  }
};

template <typename Class, typename Applier> void applyMethodsToSelf(Class &cl, Applier applier) {
