...
template <typename To> std::string proxyName() {
  return typenameLookup().getName<To>() + "Proxy";
}

template <typename To> std::string proxySequenceName() {
...
AllProxyClasses<T, SimpleBase> createProxyClasses(pybind11::module &m) {
  ...
  pybind11::class_<Proxy<ranges::optional<T>>, OptionalProxy> optional(
      m, strdup(proxyName<ranges::optional<T>>().c_str()));
  ...
  pybind11::class_<Proxy<RawIterator<T>>, IteratorProxy, SequenceProxy<T>> iterator(
      m, strdup(proxyName<Iterator<T>>().c_str()), pybind11::dynamic_attr());
  ...
  base.def(pybind11::init([](const T &val) -> Proxy<T> {
    return {std::function<T(std::any &)>{[val](std::any &) -> T { return val; }},
            {nullptr, nullptr, ProxyType::Simple}};
  }));

  iterator
      .def(pybind11::init([](const Iterator<T> &val) -> Proxy<RawIterator<T>> {
        return {std::function<RawIterator<T>(std::any &)>{[r = val.rng](std::any &) -> RawIterator<T> { return r; }},
                {nullptr, nullptr, ProxyType::Simple}};
      }))
      .def(pybind11::init([](const Proxy<RawRange<T>> &p) -> Proxy<RawIterator<T>> {
        return {std::function<RawIterator<T>(std::any &)>{[p](std::any &v) -> RawIterator<T> { return p(v); }},
                p.sourceType};
      }));

  range
      .def(pybind11::init([](const Range<T> &val) -> Proxy<RawRange<T>> {
        return {std::function<RawRange<T>(std::any &)>{[r = val.rng](std::any &) -> RawRange<T> { return r; }},
                {nullptr, nullptr, ProxyType::Simple}};
      }))
  ...
  optional.def(pybind11::init([](const Proxy<T> &p) -> Proxy<ranges::optional<T>> {
    return {std::function<ranges::optional<T>(std::any &)>{[p](std::any &v) -> ranges::optional<T> { return p(v); }},
            p.sourceType};
  }));
