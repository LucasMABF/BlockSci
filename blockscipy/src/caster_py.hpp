...
namespace pybind11 {
  namespace detail {
    ...
    template <> struct visit_helper<mpark::variant> {
      template <typename... Args> static auto call(Args &&...args) -> decltype(mpark::visit(args...)) {
        return mpark::visit(args...);
      }
    };

    template <typename... Ts> struct type_caster<mpark::variant<Ts...>> : variant_caster<mpark::variant<Ts...>> {};
    ...
    public:
      bool load(handle src, bool convert) {
        ...
        mpark::visit([&](auto address) { value = address; }, any.wrapped);
        return true;
      }
    ...
    template <typename T> struct type_caster<ranges::optional<T>> : optional_caster<ranges::optional<T>> {};
  } // namespace detail
} // namespace pybind11
