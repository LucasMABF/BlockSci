...
namespace blocksci {
  Block::Block(const RawBlock *rawBlock_, BlockHeight blockNum_, DataAccess &access_)
      : Block(rawBlock_, Transaction(access_.getChain().getTxData(rawBlock_->firstTxIndex), rawBlock_->firstTxIndex,
                                     blockNum_, static_cast<uint32_t>(access_.getChain().txCount()), access_)) {
  }

  Block::Block(BlockHeight blockNum_, DataAccess &access_)
      : Block(access_.getChain().getBlock(blockNum_), blockNum_, access_) {
  }
  ...
  std::string Block::toString() const {
    std::stringstream ss;
    ss << "Block(tx_count=" << rawBlock->txCount << ", height=" << height() << ", header_hash=" << getHeaderHash()
       << ", version=" << rawBlock->version << ", timestamp=" << rawBlock->timestamp << ", bits=" << rawBlock->bits
