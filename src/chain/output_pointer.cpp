...
namespace blocksci {
  ...
  std::vector<Transaction> getOutputTransactions(const std::vector<OutputPointer> &pointers, DataAccess &access) {
    auto txNums = pointers | ranges::views::transform([](const OutputPointer &pointer) { return pointer.txNum; }) |
                  ranges::to<std::vector>();
    txNums |= ranges::action::sort | ranges::action::unique;
    return txNums | ranges::views::transform([&access](uint32_t txNum) {
             return Transaction(txNum, access.getChain().getBlockHeight(txNum), access);
           }) |
           ranges::to<std::vector>();
  }

  std::vector<Transaction> getInputTransactions(const std::vector<OutputPointer> &pointers, DataAccess &access) {
    auto txes = pointers | ranges::views::transform([&access](const OutputPointer &pointer) {
                  return Output(pointer, access).getSpendingTx();
                }) |
                flatMapOptionals() | ranges::to<std::vector>();
