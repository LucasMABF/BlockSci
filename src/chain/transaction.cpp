...
namespace blocksci {
  ...
  Transaction::Transaction(uint32_t index, BlockHeight height, DataAccess &access_)
      : Transaction(access_.getChain().getTxData(index), index, height,
                    static_cast<uint32_t>(access_.getChain().txCount()), access_) {
  }

  Transaction::Transaction(uint32_t index, DataAccess &access_)
      : Transaction(index, access_.getChain().getBlockHeight(index), access_) {
  }

  Transaction::Transaction(const uint256 &hash, DataAccess &access_)
      : Transaction(getTxIndex(hash, access_.getHashIndex()), access_) {
  }
  ...
  std::string Transaction::toString() const {
    std::stringstream ss;
    ss << "Tx(len(txins)=" << inputCount() << ", len(txouts)=" << outputCount() << ", size_bytes=" << sizeBytes()
       << ", block_height=" << getBlockHeight() << ", tx_index=" << txNum << ")";
    return ss.str();
  }
  ...
  bool includesOutputOfType(const Transaction &tx, AddressType::Enum type) {
    return ranges::any_of(tx.outputs(), [=](const Output &output) { return output.getType() == type; });
  }
