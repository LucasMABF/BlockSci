...
namespace blocksci {
  Input::Input(const InputPointer &pointer_, DataAccess &access_)
      : Input(pointer_,                                                              // const InputPointer &pointer_
              ...
              &access_.getChain().getSpentOutputNumbers(
                  pointer_.txNum)[pointer_.inoutNum], // const uint16_t *spentOutputNum_
  ...
  std::string Input::toString() const {
    std::stringstream ss;
    ss << "TxIn(spent_tx_index=" << inout->getLinkedTxNum() << ", address=" << getAddress().toString()
