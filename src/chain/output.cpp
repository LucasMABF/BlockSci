...
namespace blocksci {
  Output::Output(const OutputPointer &pointer_, DataAccess &access_)
      : Output(pointer_, -1, access_.getChain().getTx(pointer_.txNum)->getOutput(pointer_.inoutNum),
  ...
  std::string Output::toString() const {
    std::stringstream ss;
    ss << "TxOut(spending_tx_index=" << inout->getLinkedTxNum() << ", address=" << getAddress().toString()
       << ", value=" << inout->getValue() << ")";
    return ss.str();
  }
