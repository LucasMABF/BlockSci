...
namespace blocksci {
  ...
  Blockchain::Blockchain(std::unique_ptr<DataAccess> access_)
      : BlockRange{{0, access_->getChain().blockCount()}, access_.get()}, access(std::move(access_)) {
  }

  Blockchain::Blockchain(const DataConfiguration &config) : Blockchain(std::make_unique<DataAccess>(config)) {
  }

  Blockchain::Blockchain(const std::string &configPath, BlockHeight maxBlock)
      : Blockchain(loadBlockchainConfig(configPath, true, maxBlock)) {
  }
