...
namespace blocksci {
  ...
  void TransactionRange::iterator::updateNextBlock() {
    ...
    nextBlockFirst = tx.blockHeight < tx.access->getChain().blockCount() - BlockHeight{1}
                         ? block->firstTxIndex + static_cast<uint32_t>(block->txCount)
                         : std::numeric_limits<decltype(nextBlockFirst)>::max();
  }
