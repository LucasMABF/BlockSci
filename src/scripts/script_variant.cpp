...
namespace {
  template <blocksci::AddressType::Enum type> struct ScriptCreateFunctor {
    static blocksci::ScriptVariant f(uint32_t scriptNum, blocksci::DataAccess &access) {
      return blocksci::ScriptAddress<type>(scriptNum, access.getScripts().getScriptData<dedupType(type)>(scriptNum),
                                           access);
    }
  };
...
namespace blocksci {
  ...
  AnyScript::AnyScript(const Address &address)
      : wrapped(scriptCreator.at(static_cast<size_t>(address.type))(address.scriptNum, address.getAccess())) {
  }
  AnyScript::AnyScript(uint32_t addressNum, AddressType::Enum type, DataAccess &access)
      : wrapped(scriptCreator.at(static_cast<size_t>(type))(addressNum, access)) {
  }
  ...
  EquivAddress AnyScript::getEquivAddresses(bool nestedEquivalent) const {
    return mpark::visit([&](auto &scriptAddress) { return scriptAddress.getEquivAddresses(nestedEquivalent); },
                        wrapped);
  }
