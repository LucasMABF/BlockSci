...
namespace blocksci {
  ...
  ranges::any_view<OutputPointer> Address::getOutputPointers() const {
    return access->getAddressIndex().getOutputPointers(*this) |
           ranges::views::transform(
               [](const InoutPointer &pointer) { return OutputPointer(pointer.txNum, pointer.inoutNum); });
  }
  ...
  ranges::any_view<Input> Address::getInputs() const {
    auto _access = access;
    return getOutputPointers() | ranges::views::transform([_access](const OutputPointer &pointer) {
             return Output(pointer, *_access).getSpendingInput();
           }) |
           flatMapOptionals();
  }
  ...
  ranges::any_view<Transaction> Address::getOutputTransactions() const {
    ...
    return _access->getAddressIndex().getOutputPointers(*this) |
           ...
           ranges::views::unique | ranges::views::transform([_access](uint32_t txNum) {
             return Transaction(txNum, _access->getChain().getBlockHeight(txNum), *_access);
           });
  }
  ...
  class AddressAllTxRange : public ranges::view_facade<AddressAllTxRange> {
    ...
    public:
      ...
      cursor(ranges::iterator_t<PointerView> &&it_, ranges::sentinel_t<PointerView> &&end_,
             const Address &searchAddress_, DataAccess *access_)
  ...
  ranges::optional<Address> getAddressFromString(const std::string &addressString, DataAccess &access) {
    if (addressString.compare(0, access.config.chainConfig.segwitPrefix.size(),
                              access.config.chainConfig.segwitPrefix) == 0) {
      std::pair<int, std::vector<uint8_t>> decoded =
          segwit_addr::decode(access.config.chainConfig.segwitPrefix, addressString);
      if (decoded.first == 0) {
