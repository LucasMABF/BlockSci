...
namespace {
  ...
  /**
   ...
   Note, this only returns nested addresses of a script-hash address, not the reverse (i.e., it doesn't look up
   potential wrapping addresses for non-script-hash addresses)
   */
...
namespace blocksci {
  EquivAddress::EquivAddress(const Address &address, bool scriptEquivalent_)
      : scriptEquivalent(scriptEquivalent_), access(&address.getAccess()),
        addresses(initAddresses(DedupAddress{address.scriptNum, dedupType(address.type)}, scriptEquivalent_,
                                address.getAccess())) {
  }

  EquivAddress::EquivAddress(const DedupAddress &address, bool scriptEquivalent_, DataAccess &access_)
      : scriptEquivalent(scriptEquivalent_), access(&access_),
        addresses(initAddresses(address, scriptEquivalent_, access_)) {
  }
  ...
  ranges::any_view<OutputPointer> EquivAddress::getOutputPointers() const {
    return addresses | ranges::views::transform([](const Address &address) { return address.getOutputPointers(); }) |
           ranges::views::join;
  }
  ...
  ranges::any_view<Output> EquivAddress::getOutputs() const {
    return addresses | ranges::views::transform([](const Address &address) { return address.getOutputs(); }) |
           ranges::views::join;
  }

  ranges::any_view<Input> EquivAddress::getInputs() const {
    return addresses | ranges::views::transform([](const Address &address) { return address.getInputs(); }) |
           ranges::views::join;
  }
