...
namespace blocksci {
  ...
  class ColumnIterator : public ranges::view_facade<ColumnIterator> {
    ...
    public:
      ...
      cursor(rocksdb::DB *db_, rocksdb::ColumnFamilyHandle *column_, std::vector<char> prefix)
          : db(db_), column(column_),
            it(std::shared_ptr<rocksdb::Iterator>{db->NewIterator(rocksdb::ReadOptions(), column)}),
            prefixBytes(std::move(prefix)) {
  ...
  public:
    ...
    ColumnIterator(rocksdb::DB *db_, rocksdb::ColumnFamilyHandle *column_, std::vector<char> prefix)
        : db(db_), column(column_), prefixBytes(std::move(prefix)) {
    }
  };
} // namespace blocksci
