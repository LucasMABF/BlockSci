...
namespace blocksci {
   ...
   *
   * This RocksDB database is a lookup table from both (tx hash) and (address hash) to (internal BlockSci index) for
   * those objects.
   *
  ...
  class HashIndex {
    ...
    template <typename T> ranges::optional<uint32_t> getMatch(rocksdb::ColumnFamilyHandle *handle, const T &t) {
  ...
  public:
    ...
    void addAddresses(std::vector<std::pair<typename blocksci::AddressInfo<type>::IDType, uint32_t>> rows) {
      ...
      for (const auto &pair : rows) {
        dataViews.emplace_back(MemoryView{reinterpret_cast<const char *>(&pair.first), sizeof(pair.first)},
  ...
  extern template ranges::any_view<std::pair<uint32_t, typename blocksci::AddressInfo<AddressType::PUBKEY>::IDType>>
  HashIndex::getAddressRange<AddressType::PUBKEY>();
  extern template ranges::any_view<std::pair<uint32_t, typename blocksci::AddressInfo<AddressType::PUBKEYHASH>::IDType>>
  ...
  extern template ranges::any_view<
      std::pair<uint32_t, typename blocksci::AddressInfo<AddressType::MULTISIG_PUBKEY>::IDType>>
  HashIndex::getAddressRange<AddressType::MULTISIG_PUBKEY>();
  extern template ranges::any_view<std::pair<uint32_t, typename blocksci::AddressInfo<AddressType::SCRIPTHASH>::IDType>>
