...
namespace blocksci {
  template <mio::access_mode mode = mio::access_mode::read> struct SimpleFileMapper;

  template <typename T, mio::access_mode mode = mio::access_mode::read> struct FixedSizeFileMapper;

  template <mio::access_mode mode = mio::access_mode::read, typename... T> struct IndexedFileMapper;

  template <typename U> struct add_const_ptr {
    using type = std::add_pointer_t<std::add_const_t<U>>;
  };

  template <typename... Types> struct add_const_ptr<std::tuple<Types...>> {
    using type = std::tuple<std::add_pointer_t<std::add_const_t<Types>>...>;
  };
  template <typename... Types> using add_const_ptr_t = typename add_const_ptr<Types...>::type;

  template <typename U> struct add_ptr {
  ...
  template <typename... Types> using add_ptr_t = typename add_ptr<Types...>::type;
  ...
  template <mio::access_mode mode> struct SimpleFileMapperBase {};

  template <typename MainType> class ArbitraryLengthData {
  ...
  template <> struct SimpleFileMapper<mio::access_mode::read> {
  private:
  ...
  template <> struct SimpleFileMapper<mio::access_mode::write> : public SimpleFileMapperBase<mio::access_mode::write> {
  private:
  ...
  public:
    ...
    template <typename T, typename = std::enable_if_t<std::is_trivially_copyable<T>::value>> bool write(const T &t) {
      return write(reinterpret_cast<const char *>(&t), sizeof(t));
    }

    template <typename T> bool write(const ArbitraryLengthData<T> &t) {
   ...
   *     - data file: stores all elements, eg. chain/block.dat that stores RawBlock objects; implemented as
   * SimpleFileMapper
   *
   ...
   */
  template <typename T, mio::access_mode mode> struct FixedSizeFileMapper {
  private:
  ...
  public:
    ...
    template <typename Test> std::vector<uint32_t> findAll(Test test) const {
      auto itemCount = size();
  ...
  template <size_t indexCount> using FileIndex = std::array<OffsetType, indexCount>;

  template <typename Tup, typename A, std::size_t... I> Tup tuple_cast_helper(A t1, std::index_sequence<I...>) {
    return std::make_tuple(reinterpret_cast<std::tuple_element_t<I, Tup>>(std::get<I>(t1))...);
  }

  template <typename... A, size_t N> add_const_ptr_t<std::tuple<A...>> tuple_cast(std::array<const char *, N> t1) {
  ...
  template <typename... A, size_t N> add_ptr_t<std::tuple<A...>> tuple_cast(std::array<char *, N> t1) {
    static_assert(sizeof...(A) == N, "The tuple sizes must be the same");
   ...
   *     - data file: stores all elements, eg. chain/tx_data.dat that stores RawTransaction (and Inout) elements;
   * implemented as SimpleFileMapper
   ...
   */
  template <mio::access_mode mode, typename... T> struct IndexedFileMapper {
  private:
    template <size_t indexNum> using nth_element = std::tuple_element_t<indexNum, std::tuple<T...>>;
    ...
    template <size_t indexNum = 0> OffsetType getOffset(uint32_t index) const {
  ...
  public:
    explicit IndexedFileMapper(const filesystem::path &pathPrefix)
        : dataFile(pathPrefix.str() + "_data"), indexFile(pathPrefix.str() + "_index") {
    }
    ...
    template <size_t indexNum> void write(uint32_t addressNum, const nth_element<indexNum> &t) {
      writeUpdateImp<indexNum>(addressNum, &t, sizeof(t));
    }

    template <size_t indexNum> void write(uint32_t addressNum, ArbitraryLengthData<nth_element<indexNum>> &t) {
      auto finalData = t.finalize();
      writeUpdateImp<indexNum>(addressNum, finalData, t.size());
    }

    template <size_t indexNum = 0> add_const_ptr_t<nth_element<indexNum>> getDataAtIndex(uint32_t index) const {
      assert(index < size());
    ...
    template <size_t indexNum = 0> add_ptr_t<nth_element<indexNum>> getDataAtIndex(uint32_t index) {
      assert(index < size());
      auto offset = getOffset<indexNum>(index);
      auto pointer = dataFile.getDataAtOffset(offset);
    ...
    template <size_t Z = sizeof...(T)> auto getData(std::enable_if_t<(Z > 1), uint32_t> index) const {
      assert(index < size());
      auto offsets = getOffsets(index);
    ...
    template <size_t Z = sizeof...(T)> auto getData(std::enable_if_t<(Z > 1), uint32_t> index) {
      assert(index < size());
      auto offsets = getOffsets(index);
