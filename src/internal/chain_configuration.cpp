...
namespace blocksci {
  ...
  void to_json(json &j, const ChainConfiguration &p) {
    j = json{{"coinName", p.coinName},         {"dataDirectory", p.dataDirectory.str()},
             {"pubkeyPrefix", p.pubkeyPrefix}, {"scriptPrefix", p.scriptPrefix},
             {"segwitPrefix", p.segwitPrefix}, {"segwitActivationHeight", p.segwitActivationHeight}};
  }
  ...
  ChainConfiguration ChainConfiguration::litecoin(const std::string &chainDir) {
    return {"litecoin", chainDir, {48}, {50}, "ltc", 1201536};
  }
  ...
  ChainConfiguration ChainConfiguration::litecoinRegtest(const std::string &chainDir) {
    return {"litecoin_regtest", chainDir, {111}, {58}, "rltc", 0};
  }
  ...
  ChainConfiguration ChainConfiguration::bitcoinRegtest(const std::string &chainDir) {
    return {"bitcoin_regtest", chainDir, {111}, {196}, "bcrt", 0};
  }

  ChainConfiguration ChainConfiguration::bitcoin(const std::string &chainDir) {
    return {"bitcoin", chainDir, std::vector<unsigned char>(1, 0), std::vector<unsigned char>(1, 5), "bc", 481824};
  }
  ...
  ChainRPCConfiguration ChainRPCConfiguration::bitcoin(const std::string &username, const std::string &password) {
    return {username, password, "127.0.0.1", 8332};
  }

  ChainRPCConfiguration ChainRPCConfiguration::bitcoinTestnet(const std::string &username,
                                                              const std::string &password) {
    return {username, password, "127.0.0.1", 18332};
  }
