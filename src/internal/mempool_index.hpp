...
namespace blocksci {
  ...
  struct TimestampIndex {
    ...
    explicit TimestampIndex(const filesystem::path &path, uint32_t firstTxIndex_)
        : timestampFile(path), firstTxIndex(firstTxIndex_) {
    }
  ...
  struct BlocktimeIndex {
    ...
    explicit BlocktimeIndex(const filesystem::path &path, int firstBlockNum_)
        : timestampFile(path), firstBlockNum(firstBlockNum_) {
    }
