...
namespace blocksci {
  ...
  template <typename UpdateFunc> class ProgressBar {
    uint64_t total;
  ...
  public:
    ...
    template <typename... Args> void update(uint64_t currentCount, Args &&...args) {
  ...
  template <class UpdateFunc> ProgressBar<UpdateFunc> makeProgressBar(uint64_t total, UpdateFunc updateFunc) {
