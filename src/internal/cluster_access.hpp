...
namespace blocksci {
  template <DedupAddressType::Enum type> struct ScriptClusterIndexFile : public FixedSizeFileMapper<uint32_t> {
  ...
  template <blocksci::DedupAddressType::Enum type> struct ClusterNumFunctor {
  ...
  class ClusterAccess {
    ...
    template <DedupAddressType::Enum type> friend struct ClusterNumFunctor;

    template <DedupAddressType::Enum type> uint32_t getClusterNumImpl(uint32_t scriptNum) const {
  ...
  public:
    ...
    ClusterAccess(const std::string &baseDirectory, DataAccess &access_)
        : clusterOffsetFile((filesystem::path{baseDirectory} / "clusterOffsets").str()),
          ...
          scriptClusterIndexFiles(blocksci::apply(DedupAddressType::all(),
                                                  [&](auto tag) {
                                                    std::stringstream ss;
