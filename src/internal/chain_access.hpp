...
namespace blocksci {
  ...
  class ChainAccess {
     ...
     * Example: Genesis block of BTC contains "[...] The Times 03/Jan/2009 Chancellor on brink of second bailout for
     * banks"
     */
     ...
     * Implemented as IndexedFileMapper as RawTransaction objects have variable length due to variable no. of
     * inputs/outputs.
     *
    ...
    /** Stores the version number that is stored in the first 4 bytes of every transaction on the blockchain, indexed by
     * tx number.
     *
    ...
    /** Stores the tx-internal output number of the spent output for every input, indexed by blockchain-wide input
     * number.
     *
    ...
    void setup() {
      ...
      if (_maxLoadedTx > txFile.size()) {
        std::stringstream ss;
        ss << "Block data corrupted. Tx file has " << txFile.size() << " transaction, but max tx to load is tx "
           << _maxLoadedTx;
  ...
  public:
    explicit ChainAccess(const filesystem::path &baseDirectory, BlockHeight blocksIgnored, bool errorOnReorg)
        : blockFile(blockFilePath(baseDirectory)), blockCoinbaseFile(blockCoinbaseFilePath(baseDirectory)),
          txFile(txFilePath(baseDirectory)), txVersionFile(txVersionFilePath(baseDirectory)),
          txFirstInputFile(firstInputFilePath(baseDirectory)), txFirstOutputFile(firstOutputFilePath(baseDirectory)),
          inputSpentOutputFile(inputSpentOutNumFilePath(baseDirectory)), sequenceFile(sequenceFilePath(baseDirectory)),
          txHashesFile(txHashesFilePath(baseDirectory)), blocksIgnored(blocksIgnored), errorOnReorg(errorOnReorg) {
      setup();
    }
    ...
    BlockHeight getBlockHeight(uint32_t txIndex) const {
      ...
      auto it = std::upper_bound(blockBegin, blockEnd, txIndex,
                                 [](uint32_t index, const RawBlock &b) { return index < b.firstTxIndex; });
      it--;
