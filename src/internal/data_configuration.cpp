  ...
  void checkVersion(const json &jsonConf) {
    ...
    if (versionNum != dataVersion) {
      throw std::runtime_error(
          "Error, parser data is not in the correct format. To fix you must delete the data file and rerun the parser");
    }
  }

  DataConfiguration::DataConfiguration(const std::string &configPath_, ChainConfiguration &chainConfig_,
                                       bool errorOnReorg_, BlockHeight blocksIgnored_)
      : configPath(configPath_), errorOnReorg(errorOnReorg_), blocksIgnored(blocksIgnored_), chainConfig(chainConfig_) {
