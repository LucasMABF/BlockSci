...
namespace blocksci {
  ...
  HashIndex::HashIndex(const filesystem::path &path, bool readonly) {
    ...
    for_each(AddressType::all(),
             [&](auto tag) { columnDescriptors.emplace_back(addressName(tag), rocksdb::ColumnFamilyOptions{}); });
    ...
    if (readonly) {
      rocksdb::Status s =
          rocksdb::DB::OpenForReadOnly(options, path.str().c_str(), columnDescriptors, &columnHandlePtrs, &dbPtr);
      if (!s.ok()) {
  ...
  ranges::optional<uint32_t> HashIndex::lookupAddressImpl(blocksci::AddressType::Enum type, const char *data,
                                                          size_t size) {
  ...
  ranges::any_view<std::pair<uint32_t, typename blocksci::AddressInfo<type>::IDType>> HashIndex::getAddressRange() {
    ...
    return getRawAddressRange(type) |
           ranges::views::transform([](std::pair<MemoryView, MemoryView> pair) -> std::pair<uint32_t, IDType> {
             auto &key = pair.first;
  ...
  template ranges::any_view<std::pair<uint32_t, typename blocksci::AddressInfo<AddressType::PUBKEY>::IDType>>
  HashIndex::getAddressRange<AddressType::PUBKEY>();
  template ranges::any_view<std::pair<uint32_t, typename blocksci::AddressInfo<AddressType::PUBKEYHASH>::IDType>>
  HashIndex::getAddressRange<AddressType::PUBKEYHASH>();
  template ranges::any_view<
      std::pair<uint32_t, typename blocksci::AddressInfo<AddressType::WITNESS_PUBKEYHASH>::IDType>>
  HashIndex::getAddressRange<AddressType::WITNESS_PUBKEYHASH>();
  template ranges::any_view<std::pair<uint32_t, typename blocksci::AddressInfo<AddressType::MULTISIG_PUBKEY>::IDType>>
  HashIndex::getAddressRange<AddressType::MULTISIG_PUBKEY>();
  template ranges::any_view<std::pair<uint32_t, typename blocksci::AddressInfo<AddressType::SCRIPTHASH>::IDType>>
  HashIndex::getAddressRange<AddressType::SCRIPTHASH>();
  template ranges::any_view<
