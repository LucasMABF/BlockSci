...
namespace {
  template <typename Job> void segmentWork(uint32_t start, uint32_t end, uint32_t segmentCount, Job job) {
...
namespace blocksci {
  ClusterManager::ClusterManager(const std::string &baseDirectory, DataAccess &access_)
  ...
  ClusterManager::getClusters() const {
    return ranges::views::ints(0u, clusterCount) |
           ranges::views::transform([&](uint32_t clusterNum) { return Cluster(clusterNum, *access); });
  }

  ranges::any_view<TaggedCluster>
  ClusterManager::taggedClusters(const std::unordered_map<Address, std::string> &tags) const {
  ...
  struct AddressDisjointSets {
    ...
    AddressDisjointSets(uint32_t totalSize, std::unordered_map<DedupAddressType::Enum, uint32_t> addressStarts_)
        : disjoinSets{totalSize}, addressStarts{std::move(addressStarts_)} {
    }
  ...
  template <typename ChangeFunc>
  std::vector<uint32_t> createClusters(BlockRange &chain,
                                       std::unordered_map<DedupAddressType::Enum, uint32_t> addressStarts,
                                       uint32_t totalScriptCount, ChangeFunc &&changeHeuristic, bool ignoreCoinJoin) {
  ...
  void recordOrderedAddresses(const std::vector<uint32_t> &parent, std::vector<uint32_t> &clusterPositions,
    ...
    clusterAddressesFile.write(reinterpret_cast<char *>(orderedScripts.data()),
                               static_cast<long>(sizeof(DedupAddress) * orderedScripts.size()));
  }
  ...
  void serializeClusterData(const ScriptAccess &scripts, const std::string &outputPath,
    ...
    auto recordOrdered = std::async(std::launch::async, recordOrderedAddresses, parent, std::ref(clusterPositions),
                                    scriptStarts, std::ref(clusterAddressesFile));
    ...
    clusterOffsetFile.write(reinterpret_cast<char *>(clusterPositions.data()),
                            static_cast<long>(sizeof(uint32_t) * clusterPositions.size()));
  }

  template <typename ChangeFunc>
  ClusterManager createClusteringImpl(BlockRange &chain, ChangeFunc &&changeHeuristic, const std::string &outputPath,
                                      bool overwrite, bool ignoreCoinJoin) {
    ...
    auto parent = createClusters(chain, scriptStarts, static_cast<uint32_t>(totalScriptCount),
                                 std::forward<ChangeFunc>(changeHeuristic), ignoreCoinJoin);
  ...
  ClusterManager ClusterManager::createClustering(BlockRange &chain, const heuristics::ChangeHeuristic &changeHeuristic,
                                                  const std::string &outputPath, bool overwrite, bool ignoreCoinJoin) {
  ...
  ClusterManager ClusterManager::createClustering(
      BlockRange &chain, const std::function<ranges::any_view<Output>(const Transaction &tx)> &changeHeuristic,
      const std::string &outputPath, bool overwrite, bool ignoreCoinJoin) {
