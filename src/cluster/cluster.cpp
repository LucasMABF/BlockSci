...
namespace {
  ...
  template <DedupAddressType::Enum type> struct DedupAddressTypesRangeFunctor {
...
namespace blocksci {
  ...
  auto getAddressesNested(uint32_t clusterNum, const ClusterAccess *clusterAccess) {
    ...
    return clusterAccess->getClusterScripts(clusterNum) |
           ranges::views::transform([access_](const DedupAddress &address) {
             auto header = access_->getScripts().getScriptHeader(address.scriptNum, address.type);
             uint32_t scriptNum = address.scriptNum;
             return addressTypesRange(address.type) |
                    ranges::views::filter([header](AddressType::Enum type) { return header->seenTopLevel(type); }) |
  ...
  ranges::optional<TaggedCluster>
  Cluster::getTaggedUnsafe(const std::unordered_map<blocksci::Address, std::string> &tags) const {
    bool isEmpty = [&]() {
  ...
  ranges::any_view<TaggedAddress>
  Cluster::taggedAddressesUnsafe(const std::unordered_map<blocksci::Address, std::string> &tags) const {
    return getPossibleAddresses(clusterNum, clusterAccess) |
           ranges::views::transform([&tags](Address &&address) -> ranges::optional<TaggedAddress> {
  ...
  ranges::any_view<TaggedAddress>
  Cluster::taggedAddresses(const std::unordered_map<blocksci::Address, std::string> &tags) const {
    return getPossibleAddresses(clusterNum, clusterAccess) |
           ranges::views::transform([tags](Address &&address) -> ranges::optional<TaggedAddress> {
             auto it = tags.find(address);
             if (it != tags.end()) {
  ...
  ranges::transform_view<ranges::transform_view<ranges::subrange<const DedupAddress *>, PossibleAddressesGetter>,
                         AddressRangeTagChecker>
  ...
  ranges::any_view<OutputPointer> Cluster::getOutputPointers() const {
    return getPossibleAddresses(clusterNum, clusterAccess) |
           ranges::views::transform([](auto &&address) { return address.getOutputPointers(); }) | ranges::views::join;
  }

  ranges::any_view<Output> Cluster::getOutputs() const {
    return getPossibleAddresses(clusterNum, clusterAccess) |
           ranges::views::transform([](auto &&address) { return address.getOutputs(); }) | ranges::views::join;
  }

  ranges::any_view<blocksci::Input> Cluster::getInputs() const {
    return getPossibleAddresses(clusterNum, clusterAccess) |
           ranges::views::transform([](auto &&address) { return address.getInputs(); }) | ranges::views::join;
  }
  ...
  int64_t Cluster::calculateBalance(BlockHeight height) const {
    ...
    auto balances = getDedupAddresses() | ranges::views::transform([access_, height](const DedupAddress &dedupAddress) {
                      ...
                      auto possibleAddressBalances =
                          addressTypesRange(dedupAddress.type) |
                          ranges::views::transform([&access_, scriptNum, height](AddressType::Enum addressType) {
