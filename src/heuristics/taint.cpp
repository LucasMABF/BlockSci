...
namespace blocksci {
  namespace heuristics {
    ...
    template <typename Taint> struct UntaintedInputCreator {
      Taint operator()(int64_t value);
    };

    template <> SimpleTaint UntaintedInputCreator<SimpleTaint>::operator()(int64_t value) {
      return {0, value};
    }

    template <> ComplexTaint UntaintedInputCreator<ComplexTaint>::operator()(int64_t value) {
      ComplexTaint taint;
      taint.reserve(1);
...
namespace blocksci {
  namespace heuristics {
    template <typename Taint> using TaintMap = std::map<InoutInfo, Taint>;
    ...
    void processOutput(TaintMap<Taint> &taintedInputs, std::unordered_map<OutputPointer, Taint> &taintedOutputs,
                       const Output &spendingOut, Taint &newTaintedValue) {
    ...
    template <typename Taint>
    void processTx(TaintMap<Taint> &taintedInputs, std::unordered_map<OutputPointer, Taint> &taintedOutputs,
                   const Transaction &tx, std::vector<Taint> &outputTaint) {
    ...
    std::vector<std::pair<Output, Taint>> getTaintedImpl(Func func,
                                                         std::vector<std::pair<Output, Taint>> &taintedOutputsRaw,
                                                         BlockHeight maxBlockHeight, bool taintFee) {
     ...
     */
    std::vector<std::pair<Output, SimpleTaint>> getPoisonTainted(std::vector<Output> &outputs,
                                                                 BlockHeight maxBlockHeight, bool taintFee) {
      // Poison taint function
      auto poisonTaint = [](const Transaction &tx, const std::vector<SimpleTaint> &taintedInputs,
                            std::vector<SimpleTaint> &outs, SimpleTaint &coinbaseTaint) {
     ...
     */
    std::vector<std::pair<Output, SimpleTaint>> getHaircutTainted(std::vector<Output> &outputs,
                                                                  BlockHeight maxBlockHeight, bool taintFee) {
      // Haircut taint function
      auto haircutTaint = [](const Transaction &tx, const std::vector<SimpleTaint> &taintedInputs,
                             std::vector<SimpleTaint> &outs, SimpleTaint &coinbaseTaint) {
        ...
        for (auto spendingOut : tx.outputs()) {
          ...
          auto newTaintedValue =
              std::min(static_cast<int64_t>(percentage * static_cast<double>(totalTaintedVal)), spendingOut.getValue());
