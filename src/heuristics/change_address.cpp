...
namespace blocksci {
  namespace heuristics {
    ...
    ranges::any_view<Output> ChangeHeuristicImpl<ChangeType::PeelingChain>::operator()(const Transaction &tx) const {
      ...
      return tx.outputs() |
             ranges::views::filter([](Output o) { return !o.isSpent() || isPeelingChain(*o.getSpendingTx()); }) |
             ranges::views::filter(filterOpReturn);
    }
     ...
     *
     * On the other hand, it is extremely unlikely that you receive power of ten change due to a wallet's coin
     * selection. Default for digits is 6 (i.e. it selects outputs with a value that is a multiple of 0.01 BTC)
     */
    ranges::any_view<Output> ChangeHeuristicImpl<ChangeType::PowerOfTen>::operator()(const Transaction &tx) const {
      ...
      return tx.outputs() | ranges::views::filter([value](Output o) { return o.getValue() % value != 0; }) |
             ranges::views::filter(filterOpReturn);
    }
    ...
    ranges::any_view<Output> ChangeHeuristicImpl<ChangeType::OptimalChange>::operator()(const Transaction &tx) const {
      ...
      return tx.outputs() |
             ranges::views::filter([smallestInputValue](Output o) { return o.getValue() < smallestInputValue; }) |
             ranges::views::filter(filterOpReturn);
    }

    /** If all inputs are of one address type (e.g., P2PKH or P2SH), it is likely that the change output has the same
     * type. */
    template <>
    ranges::any_view<Output> ChangeHeuristicImpl<ChangeType::AddressType>::operator()(const Transaction &tx) const {
      ...
      if (allInputsSameType) {
        return tx.outputs() | ranges::views::filter([inputType](Output o) { return o.getType() == inputType; }) |
               ranges::views::filter(filterOpReturn);
      } else {
        return ranges::views::empty<Output>;
      }
    ...
    ranges::any_view<Output> ChangeHeuristicImpl<ChangeType::Locktime>::operator()(const Transaction &tx) const {
      ...
      return tx.outputs() | ranges::views::filter([locktimeGreaterZero](Output o) {
               return !o.isSpent() || (o.getSpendingTx().value().locktime() > 0) == locktimeGreaterZero;
             }) |
             ranges::views::filter(filterOpReturn);
    }
    ...
    ranges::any_view<Output> ChangeHeuristicImpl<ChangeType::AddressReuse>::operator()(const Transaction &tx) const {
      ...
      return tx.outputs() | ranges::views::filter([inputAddresses](Output o) {
               return inputAddresses.find(o.getAddress()) != inputAddresses.end();
             }) |
             ranges::views::filter(filterOpReturn);
    }
     ...
     */
    template <>
    ranges::any_view<Output>
    ChangeHeuristicImpl<ChangeType::ClientChangeAddressBehavior>::operator()(const Transaction &tx) const {
      return tx.outputs() | ranges::views::filter([tx](Output o) {
               return o.getAddress().isSpendable() && o.getAddress().getBaseScript().getFirstTxIndex() == tx.txNum;
             }) |
             ranges::views::filter(filterOpReturn);
    }
    ...
    ranges::any_view<Output> ChangeHeuristicImpl<ChangeType::FixedFee>::operator()(const Transaction &tx) const {
      ...
      return tx.outputs() | ranges::views::filter([fee](Output o) {
               return !o.isSpent() || (o.getSpendingTx()->fee() * 1000 / o.getSpendingTx()->virtualSize()) == fee;
             }) |
             ranges::views::filter(filterOpReturn);
    }

    /** Disables change address clustering by returning an empty set. */
    template <> ranges::any_view<Output> ChangeHeuristicImpl<ChangeType::None>::operator()(const Transaction &) const {
      return ranges::views::empty<Output>;
    }
