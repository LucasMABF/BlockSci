...
namespace blocksci {
  namespace heuristics {
    std::vector<Transaction> getDeanonTxes(BlockRange &chain) {
      return chain.filter([](const Transaction &tx) { return isDeanonTx(tx); });
    }

    std::vector<Transaction> getChangeOverTxes(BlockRange &chain) {
      return chain.filter([](const Transaction &tx) { return isChangeOverTx(tx); });
    }

    std::vector<Transaction> getKeysetChangeTxes(BlockRange &chain) {
      return chain.filter([](const Transaction &tx) { return containsKeysetChange(tx); });
    }

    std::vector<Transaction> getCoinjoinTransactions(BlockRange &chain) {
      return chain.filter([](const Transaction &tx) { return heuristics::isCoinjoin(tx); });
    }

    std::pair<std::vector<Transaction>, std::vector<Transaction>>
    getPossibleCoinjoinTransactions(Blockchain &chain, int64_t minBaseFee, double percentageFee, size_t maxDepth) {
