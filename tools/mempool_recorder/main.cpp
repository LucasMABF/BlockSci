...
public:
  SaferBitcoinApi(std::string username_, std::string password_, std::string address_, int port_)
      : username(std::move(username_)), password(std::move(password_)), address(std::move(address_)), port(port_),
        bitcoinAPI(username, password, address, port) {
  }
...
public:
  MempoolRecorder(const std::string &configLocation, SaferBitcoinApi &bitcoinAPI_)
      : chain(configLocation), lastHeight(static_cast<int>(chain.size())), bitcoinAPI(bitcoinAPI_),
  ...
  void updateBlockTimes(time_t time) {
    ...
    for (auto &tip : tips) {
      ...
      while (height >= std::max(currentHeight - heightCutoff, 0) &&
             blocksSeen
                 .insert(std::pair<std::string, std::pair<BlockRecord, int>>(searchBlock, {BlockRecord{time}, height}))
                 .second) {
  ...
  void recordMempool() {
    ...
    if (newBlocks > 0) {
      ...
      std::stringstream ss;
      ss << "Added mempool data for " << newBlocks << " blocks and " << txWithTimestamp << " out of " << allTxs
         << " transactions.";
      print_msg(ss.str());
    }
  }
...
int main(int argc, char *argv[]) {
  ...
  auto cli = (clipp::value("config file", configFilePathString) % "Path to config file",
  ...
  while (!connected) {
    try {
      ...
      std::cerr << "Mempool recorder failed to connect to bitcoin node with {username = " << rpcConfig.username
                << ", password = " << rpcConfig.password << ", address = " << rpcConfig.address
