...
template <blocksci::AddressType::Enum type> struct NonDudupAddressInfo {
  uint32_t addressNum;
};
...
class AddressState {
  ...
  public:
    static constexpr auto type = scriptType;
    AddressBloomFilter(const filesystem::path &path)
        : BloomFilter(filesystem::path(path.str() + dedupAddressName(type)).str(), startingCount<scriptType>,
                      AddressFalsePositiveRate) {
    }
  };
  ...
  template <blocksci::AddressType::Enum type> void reloadBloomFilter(int sizeIncreaseRatio) {
...
public:
  ...
  template <blocksci::AddressType::Enum type,
            std::enable_if_t<!blocksci::DedupAddressInfo<dedupType(type)>::equived, int> = 0>
