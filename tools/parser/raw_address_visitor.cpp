...
template <AddressType::Enum type> struct ScriptWrapper {
  ScriptDataPointer<type> data;
};
using ScriptDataVariant = to_variadic_t<to_address_tuple_t<ScriptWrapper>, mpark::variant>;

template <AddressType::Enum type> struct ScriptDataCreateFunctor {
...
public:
  AnyScriptData(const RawAddress &address, const ScriptAccess &access)
      : wrapped(scriptDataCreator.at(static_cast<size_t>(address.type))(address.scriptNum, access)) {
  }
...
void visit(const RawAddress &address, const std::function<bool(const RawAddress &)> &visitFunc,
