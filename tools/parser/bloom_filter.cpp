...
BloomFilterData::BloomFilterData(int64_t maxItems_, double fpRate_)
    : maxItems(maxItems_), fpRate(fpRate_), m_numHashes(calculateHashes(fpRate_)),
      length(calculateLength(maxItems_, fpRate_)), addedCount(0) {
}
...
BloomFilter::BloomFilter(const std::string &path_, int64_t maxItems, double fpRate)
    : path(path_), impData(loadData(metaPath(), maxItems, fpRate)), store(storePath().str(), impData.length) {
}
