...
template <typename T, blocksci::DedupAddressType::Enum type> struct ParserIndexScriptInfo;

template <typename T> class ParserIndex {
protected:
...
public:
  ParserIndex(const ParserConfigurationBase &config_, const std::string &resultName)
      : config(config_), cachePath(config_.parserDirectory() / (resultName + ".txt")) {
    if (cachePath.exists()) {
  ...
  void updateScript(std::true_type, EquivType type, const blocksci::State &state,
                    const blocksci::ScriptAccess &scripts) {
    auto typeIndex = static_cast<size_t>(type);
    auto progress =
        blocksci::makeProgressBar(state.scriptCounts[typeIndex] - latestState.scriptCounts[typeIndex], [=]() {});
    uint32_t num = 0;
...
template <typename T> struct ParserScriptUpdater {
  ParserIndex<T> &index;
  const blocksci::State &state;
  const blocksci::ScriptAccess &scripts;

  ParserScriptUpdater(ParserIndex<T> &index_, const blocksci::State &state_, const blocksci::ScriptAccess &scripts_)
      : index(index_), state(state_), scripts(scripts_) {
  }
...
template <typename T> void ParserIndex<T>::runUpdate(const blocksci::State &state) {
