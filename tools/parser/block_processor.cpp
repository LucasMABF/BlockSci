...
BlockProcessor::BlockProcessor(uint32_t startingTxCount_, uint64_t startingInputCount, uint64_t startingOutputCount,
                               uint32_t totalTxCount_, blocksci::BlockHeight maxBlockHeight_)
    : startingTxCount(startingTxCount_), currentTxNum(startingTxCount_), currentInputNum(startingInputCount),
      currentOutputNum(startingOutputCount), totalTxCount(totalTxCount_), maxBlockHeight(maxBlockHeight_) {
}
...
template <typename ParseTag> class BlockFileReader;

#ifdef BLOCKSCI_FILE_PARSER

template <> class BlockFileReader<FileTag> : public BlockFileReaderBase {
  /** Map of (blkXXXXX.dat file number) -> pair(SafeMemReader for blkXXXXX.dat file, last tx number of this blkXXXXX.dat
   * file) */
  ...
  template <bool shouldAdvance> void nextTxImp(RawTransaction *tx, bool isSegwit) {
    try {
      ...
      std::cerr << "Failed to load tx"
                << " from block" << currentHeight << " at offset " << reader->offset() << ".\n"
                << e.what();
      throw;
    }
  }
...
template <> class BlockFileReader<RPCTag> : public BlockFileReaderBase {
  BitcoinAPI bapi;
  ...
  template <bool shouldAdvance> void nextTxImp(RawTransaction *tx, bool isSegwit) {
    if (currentHeight == 0) {
      ...
      auto scriptPubKey = blocksci::CScript()
                          << ParseHex("040184a11fa689ad5123690c81a3a49c8f13f8d45bac857fbcbc8bc4a8ead3eb4b1ff4d4614fa18d"
                                      "ce611aaf1f471216fe1b51851b4acf21b17fc45171ac7b13af")
                          << blocksci::OP_CHECKSIG;
...
public:
  BlockFileReader(const ParserConfiguration<RPCTag> &config, std::vector<BlockInfo<RPCTag>> &, uint32_t)
      : bapi(config.createBitcoinAPI()) {
  }
...
blocksci::RawBlock readNewBlock(uint32_t firstTxNum, uint64_t firstInputNum, uint64_t firstOutputNum,
                                const BlockInfoBase &block, BlockFileReaderBase &fileReader, NewBlocksFiles &files,
                                const std::function<bool(RawTransaction *&tx)> &loadFunc,
                                const std::function<void(RawTransaction *tx)> &outFunc, bool isSegwit) {
  ...
  for (uint32_t j = 0; j < block.nTx; j++) {
    ...
    // For every tx, write blockchain-wide number of the first tx input and output to file
    // (FixedSizeFileMapper<uint64_t>)
  ...
  blocksci::RawBlock blocksciBlock{firstTxNum,
                                   block.nTx,
                                   inputCount,
                                   outputCount,
                                   static_cast<uint32_t>(static_cast<int>(block.height)),
                                   block.hash,
 ...
 * Parse the output scripts (into CScriptView) of the transaction in order to identify address types and extract
 ...
 * Store information about the spent output with each input of the transaction. Then store information about each output
 * for future lookup. */
std::vector<std::function<void(RawTransaction &tx)>> ConnectUTXOsStep::steps() {
  return {[&](RawTransaction &tx) {
            // Fill UTXOState (SerializableMap<RawOutputPointer, UTXO>) with mapping tx output ->  UTXO(output.value,
            // txNum, type)
...
void backUpdateTxes(const ParserConfigurationBase &config) {
...

  {
    ...
    std::sort(updates.begin(), updates.end(), [](const auto &a, const auto &b) { return a.pointer < b.pointer; });
  }

  {
    blocksci::IndexedFileMapper<mio::access_mode::write, blocksci::RawTransaction> txFile(
        blocksci::ChainAccess::txFilePath(config.dataConfig.chainDirectory()));
...
public:
  ...
  ProcessSubStep(std::function<void(RawTransaction &)> func_, const DiscardCheckFunc &shouldDiscard_,
                 bool discardIfFull_)
      : func(std::move(func_)), shouldDiscard(shouldDiscard_), discardIfFull(discardIfFull_) {
  }
...
public:
  ...
  // AdvanceFunc
  ProcessStep(std::unique_ptr<ProcessorStep> func_, std::vector<std::unique_ptr<QueueStage>> stages_)
      : func(std::move(func_)), stages(std::move(stages_)) {
  }
...
ProcessStep makeStandardProcessStep(std::unique_ptr<ProcessorStep> &&func, const DiscardCheckFunc &advanceFuncFirst,
                                    const DiscardCheckFunc &advanceFuncSecond, bool discardIfFullFirst = false,
                                    bool discardIfFullSecond = false) {
...
struct ProcessStepQueue {
  ...
  void run() {
    for (auto &step : steps) {
      futures.push_back(std::async(std::launch::async, [&]() { step.run(); }));
    }
  }
  ...
  void waitForComplete() {
    ...
    finishedQueue.consume_all([](RawTransaction *tx) { delete tx; });
  }
};
...
std::vector<blocksci::RawBlock>
BlockProcessor::addNewBlocks(const ParserConfiguration<ParseTag> &config, std::vector<BlockInfo<ParseTag>> blocks,
                             UTXOState &utxoState, UTXOAddressState &utxoAddressState, AddressState &addressState,
  ...
  FixedSizeFileWriter<blocksci::uint256> txHashFile{
      blocksci::ChainAccess::txHashesFilePath(config.dataConfig.chainDirectory())};

  auto discardFunc = [](RawTransaction &) { return false; };

  auto progressBar = blocksci::makeProgressBar(
      totalTxCount, [=](RawTransaction &tx) { std::cout << ", Block " << tx.blockHeight << "/" << maxBlockHeight; });
  ...
  processQueue.addStep(
      makeStandardProcessStep(std::make_unique<CalculateTxHashStep>(txHashFile), discardFunc, discardFunc));

  // 1. Step: Parse the output scripts (into CScriptView) of the transaction in order to identify address types and
  // extract relevant information.
  processQueue.addStep(
      makeStandardProcessStep(std::make_unique<GenerateScriptOutputsStep>(), discardFunc, discardFunc));

  // 2. Step: Store information about the spent output with each input of the transaction. Then store information about
  // each output for future lookup.
  processQueue.addStep(
      makeStandardProcessStep(std::make_unique<ConnectUTXOsStep>(utxoState), discardFunc, discardFunc));

  /* 3. Step: Parse the input script of each input based information about the associated output script.
   *    Then store information about each output address for future lookup. */
  processQueue.addStep(
      makeStandardProcessStep(std::make_unique<GenerateScriptInputStep>(utxoAddressState), discardFunc, discardFunc));

  /* 4. Step: Attach a scriptNum to each script in the transaction. For address types which are
        deduplicated (Pubkey, ScriptHash, Multisig and their varients) use the previously allocated
        scriptNum if the address was seen before. Increment the scriptNum counter for newly seen addresses. */
  processQueue.addStep(
      makeStandardProcessStep(std::make_unique<ProcessAddressesStep>(addressState), discardFunc, discardFunc));

  /* 5. Step: Record the scriptNum for each output for later reference. Assign each spent input with
   the scriptNum of the output its spending */
  processQueue.addStep(
      makeStandardProcessStep(std::make_unique<RecordAddressesStep>(utxoScriptState), discardFunc, discardFunc));

  // 6. Step: Serialize transaction data, inputs, and outputs and write them to the txFile
  processQueue.addStep(makeStandardProcessStep(std::make_unique<SerializeTransactionStep>(txFile, linkDataFile),
                                               discardFunc, discardFunc));

  // 7. Step: Save address data into files for the analysis library
  processQueue.addStep(makeStandardProcessStep(std::make_unique<SerializeAddressesStep>(addressWriter), discardFunc,
                                               serializeAddressDiscardFunc, false, true));
  ...
  auto importer = std::async(std::launch::async, [&] {
    ...
    auto loadFinishedTx = [&](RawTransaction *&tx) { return processQueue.finishedQueue.pop(tx); };
    ...
    for (auto &block : blocks) {
      ...
      auto newBlock =
          readNewBlock(currentTxNum, currentInputNum, currentOutputNum, block, fileReader, files, loadFinishedTx,
                       outFunc, block.height >= config.dataConfig.chainConfig.segwitActivationHeight);
...
#ifdef BLOCKSCI_FILE_PARSER
template std::vector<blocksci::RawBlock>
BlockProcessor::addNewBlocks(const ParserConfiguration<FileTag> &config, std::vector<BlockInfo<FileTag>> nextBlocks,
                             UTXOState &utxoState, UTXOAddressState &utxoAddressState, AddressState &addressState,
                             UTXOScriptState &utxoScriptState);
#endif
#ifdef BLOCKSCI_RPC_PARSER
template std::vector<blocksci::RawBlock>
BlockProcessor::addNewBlocks(const ParserConfiguration<RPCTag> &config, std::vector<BlockInfo<RPCTag>> nextBlocks,
                             UTXOState &utxoState, UTXOAddressState &utxoAddressState, AddressState &addressState,
                             UTXOScriptState &utxoScriptState);
#endif
