...
template <typename CharType> std::vector<CharType> hexStringToVec(const std::string &scripthex) {
...
RawOutput::RawOutput(std::vector<unsigned char> scriptBytes_, int64_t value_)
    : scriptBytes(std::move(scriptBytes_)), value(value_) {
}

RawOutput::RawOutput(const vout_t &vout)
    : RawOutput(hexStringToVec<unsigned char>(vout.scriptPubKey.hex),
                static_cast<int64_t>(vout.value * 1e8 + (vout.value < 0.0 ? -.5 : .5))) {
}

void RawTransaction::load(const getrawtransaction_t &txinfo, uint32_t txNum_, blocksci::BlockHeight blockHeight_,
                          bool witnessActivated) {
  txNum = txNum_;
  isSegwit = witnessActivated;
  blockHeight = blockHeight_;
...
blocksci::uint256 RawTransaction::getHash(const InputView &info, const blocksci::CScriptView &scriptView,
                                          int hashType) const {
