...
public:
  ...
  struct BadSerializationFormatException : public std::runtime_error {
    BadSerializationFormatException(const std::string &path)
        : std::runtime_error("Tried to load data with bad serialization format from file " + path) {
    }
  };
  ...
  template <typename T> using without_const = std::add_pointer_t<std::remove_const_t<std::remove_pointer_t<T>>>;
