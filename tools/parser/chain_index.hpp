...
namespace blocksci {
  template <class Archive> void serialize(Archive &archive, uint256 &m) {
    archive(m.data);
  }
} // namespace blocksci
...
struct CBlockHeader {
  template <class Archive> void serialize(Archive &archive) {
    archive(nVersion, hashPrevBlock, hashMerkleRoot, nTime, nBits, nNonce);
  }
...
struct BlockInfoBase {
  ...
  template <class Archive> void serialize(Archive &archive) {
    archive(hash, header, height, size, nTx, inputCount, outputCount);
  }
};

template <typename ParseType> struct BlockInfo;

template <> struct BlockInfo<FileTag> : BlockInfoBase {
  ...
  BlockInfo(const CBlockHeader &h, uint32_t length, unsigned int numTxes, uint32_t inputCount, uint32_t outputCount,
            const ChainDiskConfiguration &config, int fileNum, unsigned int dataPos);

  template <class Archive> void serialize(Archive &archive) {
    archive(cereal::base_class<BlockInfoBase>(this), nFile, nDataPos);
  }
};

template <> struct BlockInfo<RPCTag> : BlockInfoBase {
  ...
  template <class Archive> void serialize(Archive &archive) {
    archive(cereal::base_class<BlockInfoBase>(this), tx);
  }
};
 ...
 * @see: std::vector<blocksci::RawBlock> updateChain(const ParserConfiguration<ParserTag>, blocksci::BlockHeight,
 * HashIndexCreator) in tools/parser/main.cpp
 */
template <typename ParseTag> struct ChainIndex {
