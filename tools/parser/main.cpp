...
void lockDataDirectory(const blocksci::DataConfiguration &dataConfig) {
  ...
  if (pidFile.exists()) {
    std::cout << "A PID file exists in the data directory, another parser instance might already be running. Aborting."
              << std::endl;
    exit(1);
  } else {
...
template <typename BlockType> struct ChainUpdateInfo {
  std::vector<BlockType> blocksToAdd;
  uint32_t splitPoint;
};

template <typename ParserTag>
std::vector<blocksci::RawBlock> updateChain(const ParserConfiguration<ParserTag> &config,
                                            blocksci::BlockHeight maxBlockNum, HashIndexCreator &hashDb) {
  using namespace std::chrono_literals;

  /* Load and update the persisted (serialized) ChainIndex object that contains information about all blocks (without
   * transaction data) Generate the ChainIndex if no data is available on disk.
   *
   ...
   *
   * This step represents the "Starting with chain of X blocks" and "Adding X blocks" step of the parser output
   * messages.
   */
  blocksci::BlockHeight splitPoint = [&]() {
    blocksci::ChainAccess oldChain{config.dataConfig.chainDirectory(), config.dataConfig.blocksIgnored,
                                   config.dataConfig.errorOnReorg};
    blocksci::BlockHeight maxSize =
        std::min(oldChain.blockCount(), static_cast<blocksci::BlockHeight>(chainBlocks.size()));
    ...
    if (static_cast<blocksci::BlockHeight>(oldChain.blockCount()) != splitPoint) {
      std::cout
          << "Previously parsed chain is on a different fork than the longest chain. You may need to reparse. Aborting."
          << std::endl;
      exit(1);
    }

    std::cout << "Starting with chain of " << oldChain.blockCount() << " blocks" << std::endl;
    std::cout << "Adding " << static_cast<blocksci::BlockHeight>(chainBlocks.size()) - splitPoint << " blocks"
              << std::endl;
  ...
  while (it != end) {
    ...
    // Process only as many blocks such that ~10,000,000 transactions are added in one BlockProcessor.addNewBlocks()
    // call
    ...
    // Add all just processed blocks to newBlocks as RawBlock. The blocks are in turn written to blockFile in the
...
void updateHashDB(const ParserConfigurationBase &config, HashIndexCreator &db) {
  blocksci::ChainAccess chain{config.dataConfig.chainDirectory(), config.dataConfig.blocksIgnored,
                              config.dataConfig.errorOnReorg};
...
void updateChain(const filesystem::path &configFilePath, bool fullParse) {
  ...
  // It'd be nice to do this after the indexes are updated, but they currently depend on the chain being fully updated
  {
    ...
    FixedSizeFileWriter<blocksci::RawBlock> blockFile{
        blocksci::ChainAccess::blockFilePath(config.dataConfig.chainDirectory())};
...
int main(int argc, char *argv[]) {
  ...
  auto rpcOptions =
      (clipp::option("--rpc").set(enableRPC) &
       (clipp::value("username", username) % "RPC username", clipp::value("password", password) % "RPC password",
  ...
  auto fileOptions = (clipp::option("--disk").set(enableDisk) &
  ...
  coinTypes << "coin type (supported modes)\n"
            << "                        bitcoin(disk, rpc)\n"
  ...
  auto configOptions =
      (clipp::value("coin type", coinType) % coinTypes.str(),
       ...
       fileOptions, rpcOptions) %
      "Configuration options";

  auto generateConfigCommand =
      clipp::command("generate-config").set(selected, mode::generateConfig) % "Create new BlockSci configuration";
  auto updateCommand = clipp::command("update").set(selected, mode::update) % "Update all BlockSci data";
  auto updateCoreCommand = clipp::command("core-update").set(selected, mode::updateCore) %
                           "Update just the core BlockSci data (excluding indexes)";
  auto indexUpdateCommand =
      clipp::command("index-update").set(selected, mode::updateIndexes) % "Update indexes to latest chain state";
  auto addressIndexUpdateCommand = clipp::command("address-index-update").set(selected, mode::updateAddressIndex) %
  ...
  auto compactIndexesCommand = clipp::command("compact-indexes").set(selected, mode::compactIndexes) %
                               "Compact indexes to speed up blockchain construction";
  auto doctorCommand = clipp::command("doctor").set(selected, mode::doctor) %
                       "Diagnose issues with BlockSci or the provided config file.";

  std::string configFilePathString;
  auto configFileOpt = clipp::value("config file", configFilePathString) % "Path to config file";

  auto commands = (generateConfigCommand, configOptions) | updateCommand | updateCoreCommand | indexUpdateCommand |
                  addressIndexUpdateCommand | hashIndexUpdateCommand | compactIndexesCommand | doctorCommand;
  ...
  case mode::generateConfig: {
    ...
    json jsonConf = {{"version", blocksci::dataVersion}, {"chainConfig", chainConfig}, {"parser", parser}};
