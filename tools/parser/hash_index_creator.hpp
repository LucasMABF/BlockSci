...
template <typename T, blocksci::AddressType::Enum type> struct HashIndexAddressCacheImpl;

template <typename Key> struct DenseHashMapWrappedKey {
  Key key;
  uint8_t extra;
...
namespace std {
  template <typename T> struct hash<DenseHashMapWrappedKey<T>> {
...
template <typename Key> struct DenseHashMapCache {
...
struct HashIndexAddressCacheImpl<std::true_type, type>
    : public DenseHashMapCache<typename blocksci::AddressInfo<type>::IDType> {};

template <blocksci::AddressType::Enum type> struct HashIndexAddressCacheImpl<std::false_type, type> {
  void clear() {
  }
...
template <blocksci::AddressType::Enum type>
struct HashIndexAddressCache
    : public HashIndexAddressCacheImpl<
          std::integral_constant<bool, !std::is_same<typename blocksci::AddressInfo<type>::IDType, void>::value>,
          type> {};
...
public:
  template <blocksci::AddressType::Enum type> void clearAddressCache() {
  ...
  void processTx(const blocksci::RawTransaction *tx, uint32_t txNum, const blocksci::ChainAccess &chain,
