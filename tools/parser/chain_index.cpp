...
BlockInfoBase::BlockInfoBase(const blocksci::uint256 &hash_, const CBlockHeader &h, uint32_t size_,
                             unsigned int numTxes, uint32_t inputCount_, uint32_t outputCount_)
    : hash(hash_), header(h), height(-1), size(size_), nTx(numTxes), inputCount(inputCount_),
      outputCount(outputCount_) {
}

BlockInfo<FileTag>::BlockInfo(const CBlockHeader &h, uint32_t size_, unsigned int numTxes, uint32_t inputCount_,
                              uint32_t outputCount_, const ChainDiskConfiguration &config, int fileNum,
                              unsigned int dataPos)
    : BlockInfoBase(config.workHashFunction(reinterpret_cast<const char *>(&h), sizeof(CBlockHeader)), h, size_,
                    numTxes, inputCount_, outputCount_),
      nFile(fileNum), nDataPos(dataPos) {
}
...
BlockInfo<RPCTag>::BlockInfo(const blockinfo_t &info, blocksci::BlockHeight height_)
    : BlockInfoBase(blocksci::uint256S(info.hash),
                    {info.version, blocksci::uint256S(info.previousblockhash), blocksci::uint256S(info.merkleroot),
                     info.time, 0, info.nonce},
                    static_cast<uint32_t>(info.size), static_cast<uint32_t>(info.tx.size()), 0, 0),
      tx(info.tx) {
  height = height_;
}
...
namespace {
  std::vector<BlockInfo<FileTag>> readBlocksImpl(SafeMemReader &reader, int fileNum,
                                                 const ChainDiskConfiguration &config) {
    try {
      ...
      std::cerr << "Failed to read block header information"
                << " from " << reader.getPath() << " at offset " << reader.offset() << ": " << e.what() << "\n";
      throw;
    }
  }
} // namespace
...
template <> void ChainIndex<FileTag>::update(const ConfigType &config, blocksci::BlockHeight /*maxblockHeight*/) {
  int fileNum = 0;
  ...
  std::atomic<int> activeThreads{0};
  {
    ...
    for (; fileNum <= maxFileNum; fileNum++) {
      ...
      blockFutures.emplace_back(std::async(
          std::launch::async,
          [&](int fileNum) {
            activeThreads++;
            ...
            filesDone++;
            std::cout << "\r" << (static_cast<double>(filesDone) / static_cast<double>(fileCount)) * 100
                      << "% done fetching block headers" << std::flush;
          },
          fileNum));
    }
  }
...
template <> void ChainIndex<RPCTag>::update(const ConfigType &config, blocksci::BlockHeight blockHeight) {
  try {
    ...
    for (blocksci::BlockHeight i = splitPoint; i < blockHeight; i++) {
      ...
      if (count % percentageMarker == 0) {
        std::cout << "\r"
                  << (static_cast<double>(static_cast<int>(count)) / static_cast<double>(static_cast<int>(numBlocks))) *
                         100
                  << "% done fetching block headers" << std::flush;
      }
      if (i == blockHeight - 1) {
