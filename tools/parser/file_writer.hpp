...
public:
  ...
  template <typename T, typename = std::enable_if_t<std::is_trivially_copyable<T>::value>> void writeImp(const T &t) {
  ...
  template <typename K> void update(size_t offset, const K &t) {
...
struct ArbitraryFileWriter : SimpleFileWriter {
  ...
  template <class It> void write(It it, It end) {
...
public:
  ...
  template <typename K> void updateData(uint32_t index, size_t offset, const K &t) {
...
template <size_t indexCount> using FileIndex = std::array<uint64_t, indexCount>;

template <size_t indexCount> struct IndexedFileWriter {
private:
...
public:
  explicit IndexedFileWriter(const filesystem::path &pathPrefix)
      : dataFile(filesystem::path{pathPrefix.str() + "_data"}),
        indexFile(filesystem::path{pathPrefix.str() + "_index"}) {
  }
  ...
  template <size_t indexNum> void updateIndexGroup(uint32_t addressNum) {
