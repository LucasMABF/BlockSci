...
HashIndexCreator::HashIndexCreator(const ParserConfigurationBase &config_, const filesystem::path &path)
    : ParserIndex(config_, "hashIndex"), db(path, false) {
}

template <bool, blocksci::AddressType::Enum type> struct ClearerFunctor;

template <blocksci::AddressType::Enum type> struct ClearerFunctor<true, type> {
...
template <blocksci::AddressType::Enum type> struct ClearerFunctor<false, type> {
...
HashIndexCreator::~HashIndexCreator() {
  ...
  for_each(blocksci::AddressType::all{}, [&](auto tag) {
    ClearerFunctor<!std::is_same<typename blocksci::AddressInfo<tag.value>::IDType, void>::value, tag.value>{
        std::get<HashIndexAddressCache<tag.value>>(addressCache), db}();
  });
}

void HashIndexCreator::processTx(const blocksci::RawTransaction *tx, uint32_t txNum, const blocksci::ChainAccess &chain,
