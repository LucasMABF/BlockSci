...
struct BloomFilterData {
  ...
  template <class Archive> void serialize(Archive &archive) {
    archive(maxItems, fpRate, m_numHashes, length, addedCount);
  }
};
...
public:
  ...
  template <class Key> void add(const Key &key) {
    int len = static_cast<int>(sizeof(Key));
    auto item = reinterpret_cast<const uint8_t *>(&key);
    add(item, len);
  }

  template <class Key> bool possiblyContains(const Key &key) const {
