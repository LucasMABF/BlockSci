...
struct ParserConfigurationBase {
  ...
  /* Directory that stores the serialization of the UTXOAddressState class. For each address type, this contains mapping
     from output pointers to addresses of that type to data necessary to parse the input script spending an output of
     that type */
  filesystem::path utxoAddressStatePath() const {
    return parserDirectory() / "utxoAddressState";
  }

  // File that contains the serialization of the UTXOScriptState class which maps output pointers to the scriptNum of
  // the containted script
  filesystem::path utxoScriptStatePath() const {
    return parserDirectory() / "utxoScriptState.dat";
  }

  /* Directory that contains the serialization of the AddressState class. This contains bloom filters which provide
     checking of whether an address has been seen before, multi-address maps providing fast lookups for addresses seen
     multiple times, and a count of the total number of scripts of each type */
   ...
   * BlockType contains block data like the hash, height, size, no. of txes, inputCount, outputcount, CBlockHeader
   * object
   */
...
struct ChainDiskConfiguration {
  ...
  ChainDiskConfiguration(const std::string bitcoinDir, uint32_t blockMagic_, std::string hashFuncName)
      : coinDirectory(bitcoinDir), blockMagic(blockMagic_), hashFuncName(std::move(hashFuncName)) {
    resetHashFunc();
  }
...
template <> struct ParserConfiguration<FileTag> : public ParserConfigurationBase {
  ParserConfiguration();
...
template <> struct ParserConfiguration<RPCTag> : public ParserConfigurationBase {
  ParserConfiguration();
