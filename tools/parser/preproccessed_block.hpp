...
struct RawTransaction {
  ...
  RawTransaction() : txNum(0), hash(), baseSize(0), realSize(0), locktime(0), version(0), blockHeight(0) {
  }
...
#ifdef BLOCKSCI_RPC_PARSER
  void load(const getrawtransaction_t &txinfo, uint32_t txNum, blocksci::BlockHeight blockHeight,
