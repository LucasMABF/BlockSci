...
template <DedupAddressType::Enum dedupType> uint256 compute_scriptdata_hash(const DataAccess &access);

/**
 Compute a checksum over pubkey data.
 */
template <> uint256 compute_scriptdata_hash<DedupAddressType::Enum::PUBKEY>(const DataAccess &access) {
  Sha256Stream stream;
...
*/
template <> uint256 compute_scriptdata_hash<DedupAddressType::Enum::SCRIPTHASH>(const DataAccess &access) {
  Sha256Stream stream;
...
*/
template <> uint256 compute_scriptdata_hash<DedupAddressType::Enum::MULTISIG>(const DataAccess &access) {
  Sha256Stream stream;
...
*/
template <> uint256 compute_scriptdata_hash<DedupAddressType::Enum::NULL_DATA>(const DataAccess &access) {
  Sha256Stream stream;
...
*/
template <> uint256 compute_scriptdata_hash<DedupAddressType::Enum::NONSTANDARD>(const DataAccess &access) {
  Sha256Stream stream;
...
*/
template <> uint256 compute_scriptdata_hash<DedupAddressType::Enum::WITNESS_UNKNOWN>(const DataAccess &access) {
  Sha256Stream stream;
 ...
 */
template <AddressType::Enum type> void hash_addressrange(Sha256Stream &stream, const DataAccess &access) {
    ...
    if (data->hasWrappedAddress()) {
      ...
      if (nestingAddresses.size() > 0) {
        if (std::find(nestingAddresses.begin(), nestingAddresses.end(), expectedAddress) == nestingAddresses.end()) {
          ...
          std::cout << "Incorrect nesting scripthash for (" << wrappedAddress.scriptNum << ", " << wrappedAddress.type
                    << ")." << std::endl;
        }
      } else {
        allNestingsCorrect = false;
        std::cout << "Found no nesting scripthash for (" << wrappedAddress.scriptNum << ", " << wrappedAddress.type
                  << "). Expected: (" << i << ", " << dedupType << ")." << std::endl;
      }
    }
  }
...
int main(int argc, char *argv[]) {
  ...
  std::cout << "Chain contains " << chain.size() << " blocks, " << chain.getAccess().getChain().txCount() << " txes, "
            << chain.getAccess().getChain().inputCount() << " inputs, " << chain.getAccess().getChain().outputCount()
            << " outputs." << std::endl;
