...
int main(int argc, char *argv[]) {
  ...
  auto version1 =
      timeFunc("versionGreaterOneSingleThreaded", calculateVersionGreaterOneSingleThreaded, iterations, chain);
  auto version2 =
      timeFunc("versionGreaterOneMultithreaded", calculateVersionGreaterOneMultithreaded, iterations, chain);
  ...
  if (includeTraversal) {
    uniqueLocktimeSingle =
        timeFunc("uniqueLocktimeChangeSingleThreaded", calculateUniqueLocktimeChangeSingleThreaded, iterations, chain);
    uniqueLocktimeMulti =
        timeFunc("uniqueLocktimeChangeMultithreaded", calculateUniqueLocktimeChangeMultithreaded, iterations, chain);
  ...
  if (satoshiDiceAddress) {
    maxSatoshiDiceOutput = timeFunc("satoshiDiceTotalOutputValueSingleThreaded", calculateSatoshiDiceTotalOutputValue,
                                    iterations, chain, satoshiDiceAddress->scriptNum, satoshiDiceAddress->type);
  }
...
int64_t calculateMaxFeeMultithreaded(BlockRange &chain) {
  auto extract = [](const Transaction &tx) { return fee(tx); };
...
uint32_t calculateNonzeroLocktimeMultithreaded(BlockRange &chain) {
  auto extract = [](const Transaction &tx) { return static_cast<uint32_t>(tx.locktime() > 0); };

  auto combine = [](uint32_t &a, uint32_t &b) -> uint32_t & {
    a += b;
    return a;
  };
...
uint32_t calculateVersionGreaterOneMultithreaded(BlockRange &chain) {
  auto extract = [](const Transaction &tx) { return static_cast<uint32_t>(tx.getVersion() > 1); };
